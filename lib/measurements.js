...
async function doMeasurementsYear(adapter, anlagenId, year, months, wallbox) {
	adapter.log.debug(`🔄 Reading measurements for year: ${year}${months ? ".monthly" : ""}`);
	...
	if (year !== new Date().getUTCFullYear()) {
		...
		if (
			...
			adapter.log.silly(`Measurements for ${year}${months ? ".monthly" : ""} already updated today. Skipping.`);
			return { status: "skipped_existing" };
		}
	}
	...
	adapter.log.debug(`🔄 Polling measurements for ${url}`);
...
async function doMeasurementsMonth(adapter, anlagenId, date, period, wallbox) {
	adapter.log.debug(`🔄 Reading measurements for ${period}.`);
	...
	if (period === "previous_month" || period === "previous_month.daily") {
		...
		if (lastUpdate && lastUpdate.val !== null && lastUpdate.val !== undefined) {
			...
			if (
				...
				adapter.log.silly(`Measurements for ${period} already updated this month. Skipping.`);
				return { status: "skipped_existing" };
			}
		}
	}
	...
	if (period === "yesterday" || period === "yesterday.hourly") {
		...
		if (lastUpdate && lastUpdate.val !== null && lastUpdate.val !== undefined) {
			...
			if (
				...
				adapter.log.silly(`Measurements for ${period} already updated today. Skipping.`);
				return { status: "skipped_existing" };
			}
		}
	}
...
async function _fetchAndSumMeasurements(adapter, url, anlagenId, pfx, period, logLabel) {
	...
	if (ts.length === 0) {
		adapter.log.debug(`No measurements found for ${logLabel}.`);
...
async function doSumMeasurements(adapter, data, anlagenId, pfx, period) {
	adapter.log.debug(`Measurements sample: ${JSON.stringify(data).slice(0, 500)}`);
	...
	adapter.log.silly(`Sums: ${JSON.stringify(sums)}`);
	let groupBy;
	switch (period) {
		case "year":
			groupBy = year;
