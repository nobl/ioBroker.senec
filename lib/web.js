...
function SenecWeb(server, webSettings, adapter, instanceSettings, app) {
	...
	// HTTPS agent for SENEC device (limited concurrent connections)
	const agent = new https.Agent({ rejectUnauthorized: true, maxSockets: 3 });
	...
	app.get("/senec/api/log", (req, res) => {
		// Require authentication when the web adapter has auth enabled
		if (webSettings.auth && !req.isAuthenticated()) {
			return safeJson(res, 401, { error: "Unauthorized. Please log in." });
		}
