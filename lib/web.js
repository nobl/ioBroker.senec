 ...
 * It serves static files at /senec/ and provides a welcome page entry.
 *
...
function SenecWeb(server, webSettings, adapter, instanceSettings, app) {
	// express is provided by ioBroker.web at runtime — not a direct dependency
	// @ts-expect-error no express types in this project
	...
	app.use("/senec", express.static(wwwDir));

	adapter.log.info("[SENEC] Web extension loaded, serving dashboard at /senec/");
		...
		return new Promise((resolve) => {
			if (app._router && app._router.stack) {
				...
				app._router.stack = app._router.stack.filter(
					(layer) =>
						!(layer.route && layer.route.path === "/senec/api/log") &&
						!(layer.name === "serveStatic" && layer.regexp && layer.regexp.test("/senec")),
				);
