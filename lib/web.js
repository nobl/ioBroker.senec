...
function SenecWeb(server, webSettings, adapter, instanceSettings, app) {
	...
	// Pooled connections to the device, mirroring the local connector's agent options.
	// The agent timeout only expires idle pooled sockets — the per-request timeout still governs
	// how long a single request may take.
	const AGENT_OPTS = { keepAlive: true, maxSockets: 3, timeout: 60000 };

	/**
	 * Replace the HTTPS agent, destroying the previous one first.
	 * Required with keepAlive: an orphaned agent would hold idle sockets to the device open.
	 *
	 * @param {object} opts - Agent options merged over AGENT_OPTS
	 */
	const setAgent = (opts) => {
		if (agent) {
			agent.destroy();
		}
		agent = new https.Agent({ ...AGENT_OPTS, ...opts });
	};
	...
	const resolveAgent = async () => {
		...
		if (mode === "tofu") {
			...
			if (fp && fp.length > 10) {
				...
				setAgent({ rejectUnauthorized: false }); // CodeQL: intentional — TOFU fingerprint validation provides identity verification
				agentResolved = true;
	...
	this.unload = function () {
		return new Promise((resolve) => {
			// Close pooled connections to the device
			if (agent) {
				agent.destroy();
				agent = null;
				agentResolved = false;
