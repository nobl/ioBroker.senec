/**
 * Value labels are transcribed from the appliance's own language files (DE-de.js, EN-en.js,
 * IT-it.js, served by the device under /js/), so that what this adapter publishes matches what
 * the appliance itself displays.
 *
 * Three English labels deliberately DIVERGE from that source, because the appliance ships them
 * misspelled and there is nothing to be gained by reproducing a vendor typo in our own output:
 *   code  2 — appliance "ERROR ELECTRICY METER", here ELECTRICITY
 *   code 37 — appliance "NSP ERROR: HARDWRE",   here HARDWARE
 *   code 52 — appliance "MAN: EQUALIZATION: READY", here "MAN." to match its own siblings 23,
 *             24, 45, 46, 47, 50 and 51, which the appliance already writes with a period
 * Do not "re-sync" these three from the device without reading this note first. The German
 * table's "MAN:" at codes 50-52 is NOT one of these: the appliance really does write it that
 * way, and it is left alone on purpose.
 */

/**
 * Cascade state per system. casc_enum exists only in DE-de.js, where its values are untranslated
 * English identifiers; EN-en.js and IT-it.js define no table for it at all. One table therefore
 * serves all three languages — kept as a single object so the copies cannot drift apart.
 */
const CASC_STATE = {
	0: "PIDmax (0)",
	1: "Full (1)",
...
/**
 * Battery/inverter model the system was built with. Every value is a product name, so there is
 * nothing to translate and one table serves all three languages — kept as a single object so the
 * copies cannot drift apart.
 */
const FACTORY_BAT_TYPE = {
	0: "Studer Xtender (0)",
	1: "SenecBatt (1)",
...
const state_trans = {
	"ENERGY.STAT_STATE.0": {
		...
		74: "FACTORY SETTINGS NICHT GEFUNDEN (74)",
	...
	"ENERGY.STAT_STATE.2": {
		0: "STATO INIZIALE",
		1: "ERRORE COMUNICAZIONE INVERTER",
		2: "ERRORE WATTMETRO",
		...
		64: "MODALITÀ DI PROTEZIONE LITIO ATTIVA",
		65: "MODALITÀ DI PROTEZIONE LITIO COMPLETA",
		66: "ERRORE DI TENSIONE BATTERIA",
		67: "BMS DC SPENTO",
		...
		77: "ERRORE DI FUNZIONAMENTO ISOLATO",
		78: "INIZIALIZZAZIONE",
		79: "MODALITÀ INSTALLAZIONE",
		80: "RETE OFFLINE",
		...
		98: "ERRORE BMS - SQUILIBRIO DEL MODULO (98)",
		99: "CARICA DI RISVEGLIO (99)",
		100: "SOX-CALIBRAZIONE (100)",
	},
	"FACTORY.BAT_TYPE.0": FACTORY_BAT_TYPE,
	"FACTORY.BAT_TYPE.1": FACTORY_BAT_TYPE,
	"FACTORY.BAT_TYPE.2": FACTORY_BAT_TYPE,
	"FACTORY.COUNTRY.0": {
		0: "Deutschland - DE (0)",
		1: "Italien - IT (1)",
		2: "Australien - AU (2)",
	...
	"SYS_UPDATE.FSM_STATE.0": {
		0: "Melde Systemstart am Server (0)",
		1: "Warte auf nächste Anfrage (1)",
		2: "Melde Zustand am Server (2)",
		3: "Steuerungssoftware Download (3)",
		4: "Prüfsumme Download (4)",
		5: "Prüfsummen Vergleich (5)",
		6: "Weboberfläche Download (6)",
		7: "Prüfsumme Download (7)",
		8: "Prüfsummen Vergleich & Entpacke Weboberfläche (8)",
		9: "System Neustart - Seite in 300 Sekunden neu laden. (9)",
	},
	"SYS_UPDATE.FSM_STATE.1": {
		0: "Logging systemboot to server (0)",
		1: "Waiting for next request (1)",
		2: "Reporting status to server (2)",
		3: "Controller-software Download (3)",
		4: "Checksum Download (4)",
		5: "Checksum Validation (5)",
		6: "GUI Download (6)",
		7: "Checksum Download (7)",
		8: "Checksum Validation & Extracting GUI (8)",
		9: "System Reboot - Reload page in 300 seconds. (9)",
	},
	"SYS_UPDATE.FSM_STATE.2": {
		0: "Notifica al server: riavvio del sistema (0)",
		1: "In attesa di richiesta successiva (1)",
		2: "Notifica al server: stato (2)",
		3: "Download del software (3)",
		4: "Download del checksum in corso (4)",
		5: "Controllo del checksum in corso (5)",
		6: "Download dell'interfaccia web (6)",
		7: "Download del checksum in corso (7)",
		8: "Controllo del checksum ed estrazione della GUI (8)",
		9: "Riavvia il sistema - Ricarica pagina fra 300 secondi. (9)",
	},
	"PWR_UNIT.TYPE.0": {
		0: "Undefiniert (0)",
		1: "Solarinvert SHKW (1)",
		2: "SENEC.Heat (2)",
	},
	"PWR_UNIT.TYPE.1": {
		0: "N/A (0)",
	...
	// Which meter each power unit is measured by. Wording from enfluri_name in the
	// device's own DE-de.js / EN-en.js / IT-it.js, with the UI's &nbsp; and trailing
	// colons removed.
	"PWR_UNIT.ENFLURI.0": {
		0: "Undefiniert (0)",
		1: "ENFLURI - NETZ (1)",
		2: "ENFLURI - VERBRAUCH (2)",
		3: "ENFLURI - ERZEUGUNG (3)",
	},
	"PWR_UNIT.ENFLURI.1": {
		0: "N/A (0)",
		1: "METER - GRID (1)",
		2: "METER - CONSUMPTION (2)",
		3: "METER - GENERATION (3)",
	},
	"PWR_UNIT.ENFLURI.2": {
		0: "Non definito (0)",
		1: "WATTMETRO - RETE (1)",
		2: "CONSUMI DOMESTICI (2)",
		3: "PRODUZIONE DIREZIONE FLUSSO (3)",
	},
	"CASC.STATE.0": CASC_STATE,
	"CASC.STATE.1": CASC_STATE,
	"CASC.STATE.2": CASC_STATE,
	// Battery module generation. Unlike the tables above there is no enum for this in the
	// device's language files; the codes come from the detection wizard in senec.min.js,
	// which dispatches 2 to the BMZ wizard and 4 to the Ampace one and treats anything not
	// greater than 0 as "nothing detected". Only those two are established, so any other
	// value falls through to doDecode's "(unknown)" rather than being guessed at. The
	// manufacturer names are brands and stay untranslated.
	"BMS.MANUFACTURER.0": {
		0: "Nicht erkannt (0)",
		2: "BMZ (2)",
		4: "Ampace / LFP (4)",
