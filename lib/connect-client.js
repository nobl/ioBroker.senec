...
/** Section carrying the wallboxes of a system. Each entry has its own `id`. */
const WALLBOX_SECTION = "evse";

/** Bounds for connect_timeout, mirroring the min/max the admin field enforces. */
const CONNECT_TIMEOUT_MIN_MS = 5000;
const CONNECT_TIMEOUT_MAX_MS = 120000;
const CONNECT_TIMEOUT_DEFAULT_MS = 30000;

/**
 * Resolve the request timeout for the SENEC.Connect client.
 *
 ...
 * is pulled back into it: a zero would disable the timeout and hang the poll cycle.
 *
 * @param {unknown} value - Configured value, if any
 * @returns {number} Timeout in milliseconds
 */
function resolveConnectTimeout(value) {
	const requested = Number(value);
	if (!isFinite(requested) || requested <= 0) {
		return CONNECT_TIMEOUT_DEFAULT_MS;
	}
	return Math.min(Math.max(requested, CONNECT_TIMEOUT_MIN_MS), CONNECT_TIMEOUT_MAX_MS);
}
...
function buildInclude(configured) {
	const wanted = new Set(
		// Same default as the admin field and io-package. They diverged, so clearing the field
		// in Admin silently dropped evse rather than restoring the documented default.
		String(configured || "battery,meter,evse,bessNameplate")
			.split(",")
...
/**
 * Store the wallboxes of a system under their own ids instead of their array position.
 *
 * `evse` is an array like the systems array, with the same absence of a documented order, and
 * its entries carry an `id`. Left positional, two wallboxes swap their states — and their
 * recorded history — whenever the response order changes.
 *
 * @param {ConnectSystem | null | undefined} system - One entry of the response array
 * @returns {{ system: ConnectSystem, keys: string[] }} System with keyed wallboxes, and their segments
 */
function keyWallboxes(system) {
	const list = system?.[WALLBOX_SECTION];
	if (!Array.isArray(list)) {
		return { system: system || {}, keys: [] };
	}
	const used = new Set();
	const keyed = /** @type {Record<string, unknown>} */ ({}); // eslint-disable-line jsdoc/check-tag-names
	const keys = [];
	list.forEach((wallbox, index) => {
		let key = (wallbox && typeof wallbox === "object" ? identitySegment(wallbox.id) : null) || String(index);
		if (used.has(key)) {
			let suffix = 2;
			while (used.has(`${key}-${suffix}`)) {
				suffix++;
	...
	return { system: { ...system, [WALLBOX_SECTION]: keyed }, keys };
}
...
async function pruneConnectSystems(adapter, activeKeys) {
	...
	for (const id in allObjects) {
		...
		if (!activeKeys.has(parts[0])) {
			toDelete.push(id);
			continue;
		}
		// A wallbox that left the array. Without this its states stay behind at their last
		// values, indistinguishable from a wallbox that is simply idle.
		if (parts[1] === WALLBOX_SECTION && parts.length > 2) {
			if (!activeKeys.has(`${parts[0]}.${WALLBOX_SECTION}.${parts[2]}`)) {
				toDelete.push(id);
			}
		}
	}
	...
	// Name what was retired as a whole — a system, or one system's wallbox. Everything else in
	// the list is a state underneath one of those, and listing those would drown the message.
	const retired = new Set();
	for (const id of toDelete) {
		const parts = id.slice(base.length).split(".");
		if (parts.length === 1) {
			retired.add(parts[0]);
		} else if (parts[1] === WALLBOX_SECTION && parts.length === 3) {
			retired.add(parts.join("."));
		}
	}
	let failed = 0;
	for (const id of toDelete) {
	...
	const named = retired.size > 0 ? ` — retired: ${[...retired].join(", ")}` : "";
	adapter.log.info(`[Connect] Removed ${toDelete.length - failed} object(s) no longer reported${named}`);
	if (failed > 0) {
	...
	try {
		...
		if (!adapter.connectClient) {
			adapter.connectClient = axios.create({
				// Its own budget, not the local appliance's: pollingTimeout is the LAN device
				// timeout, capped at ten seconds and only visible in Admin when the local
				// connector is on, so a Connect-only user could neither see nor raise it.
				timeout: resolveConnectTimeout(adapter.config.connect_timeout),
				// Without this, unload cannot cancel a request that is already in flight, and
				// the response lands in the middle of teardown.
				signal: adapter.abortController?.signal,
				headers: {
					"Ocp-Apim-Subscription-Key": subscriptionKey,
				},
			});
			adapter.applyDefaultHeaders(adapter.connectClient, adapter.buildUserAgent());
		}
		...
		if (response?.data && Array.isArray(response.data)) {
			...
			const keys = new Set();
			// Everything the response accounts for: the systems, plus each system's wallboxes as
			// "<system>.evse.<wallbox>". Cleanup works from this, so a wallbox that leaves the
			// array is retired just like a system that leaves the account.
			...
			for (let i = 0; i < response.data.length; i++) {
				...
				const keyedSystem = keyWallboxes(system);
				for (const wallbox of keyedSystem.keys) {
					active.add(`${key}.${WALLBOX_SECTION}.${wallbox}`);
				}
				await adapter.evalPoll(keyedSystem.system, `_connect.Systems.${key}.`);
			}
			...
			await reconcileSystems(adapter, active, anyFallback);
			...
			// HTTP 200 carrying something that is not the systems array — a captive portal, an
			// error page, an APIM fault object. Reporting the connector as connected on that
			// would leave "connected" true forever while nothing is ingested.
			adapter.log.warn(
