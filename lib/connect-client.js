...
async function connectPoll(adapter) {
	...
	if (!subscriptionKey) {
		adapter.log.warn("SENEC.Connect: No subscription key configured. Skipping poll.");
		return;
	}
	...
	try {
		...
		adapter.logError(error, "❌ SENEC.Connect poll failed");
	}

	if (!adapter.unloaded) {
		adapter.setTimeout(() => {
			connectPoll(adapter).catch((e) => adapter.logError(e, "❌ SENEC.Connect scheduled poll failed"));
		}, interval);
