...
async function connectPoll(adapter) {
	...
	try {
		adapter.log.debug("[Connect] 🔄 Polling SENEC.Connect API...");
		...
		if (response?.data && Array.isArray(response.data)) {
			...
			adapter.log.debug(`[Connect] Polled ${response.data.length} system(s)`);
		} else {
			adapter.log.warn(
	...
	if (!adapter.unloaded) {
		...
		adapter.log.debug(`[Connect] ⏱ Next SENEC.Connect poll scheduled in ${(interval / 1000).toFixed(0)}s`);
	}
}
