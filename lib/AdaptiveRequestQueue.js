...
class AdaptiveRequestQueue {
	 ...
	 * @param {{ maxRetries?: number, label?: string, shouldRetry?: (err: Error & { code?: string; response?: { status?: number } }) => boolean, isSuccess?: (result: any) => boolean }} [options] - retry options, plus an optional check for callers whose resolved value can still represent a refusal
	 */
			...
			while (this.running < this.concurrency && this.queue.length > 0) {
				...
				(async () => {
					try {
						...
						if (item.isSuccess && !item.isSuccess(result)) {
							this.stats.failed++;
