...
class AdaptiveRequestQueue {
	 ...
	 * @param {{ maxRetries?: number, label?: string, shouldRetry?: (err: Error & { code?: string; response?: { status?: number } }) => boolean }} [options] - retry options
	 */
	 ...
	 *
	 * @param {string|number|boolean|null} [retryAfter] - Value of the Retry-After header, in seconds
	 */
	_applyCooldown(retryAfter) {
		const retryAfterHeader = parseInt(String(retryAfter ?? ""), 10);
	...
	/**
	 * Record rate limiting that arrived as an ordinary response rather than as an error.
	 *
	 * A caller using axios `validateStatus: () => true` gets an HTTP 429 back as a perfectly
	 * successful response, so the queue would otherwise count it as a win, keep its
	 * concurrency and carry on hammering the server. Calling this restores the cooldown and
	 * the statistic that the thrown-error path applies by itself.
	 *
	 * Only 429 acts, matching the error path — a 5xx does not trigger backoff there either.
	 * The success bookkeeping that follows the task nudges the success streak back to 1; with
	 * a threshold of 8 that cannot lift concurrency, and the cooldown is already in force.
	 *
	 ...
	 * @returns {boolean} True when the response was treated as an overload signal
	 */
	noteRateLimited(status, retryAfter) {
		if (status !== 429) {
			return false;
		}
