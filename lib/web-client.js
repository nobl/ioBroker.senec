...
async function webInit(adapter) {
	...
	if (!adapter.webAuthenticated) {
		adapter.log.warn("[Web] Web login failed. Check credentials.");
		return;
	}
	...
	if (adapter.webMasterPlantNumber === null) {
		adapter.log.warn("[Web] No master system found.");
		return;
	}
...
async function webHandleControl(adapter, subId, state) {
	...
	if (pn === null || pn === undefined) {
		adapter.log.warn("[Web] No master plant number, cannot send control command");
		return;
	}
	...
	adapter.log.warn(`[Web] Unknown web control: ${subId}`);
}
...
async function webHandlePeakShavingApply(adapter) {
	...
	if (!mode) {
		adapter.log.warn("[Web] Peak shaving mode is empty, not applying");
...
async function webHandleSocketControl(adapter, idx, field, state) {
	...
	if (!adapter.webSocketData || !Array.isArray(adapter.webSocketData)) {
		adapter.log.warn("[Web] No socket data available, cannot apply changes");
	...
	if (!socket) {
		adapter.log.warn(`[Web] Socket ${idx} not found in stored data`);
		await adapter.setStateAsync(`${pfx}.Apply`, { val: false, ack: true });
		return;
	}
...
async function webLogin(adapter) {
	...
	if (!email || !password) {
		adapter.log.warn("[Web] No credentials configured (api_mail/api_pwd).");
		return false;
	}
	...
	try {
		...
		if (!formAction) {
			...
			adapter.log.warn("[Web] Could not find login form action URL.");
		...
		if (hasOtp(loginHtml)) {
			...
			if (!totpSecret) {
				adapter.log.warn("[Web] TOTP required but no TOTP secret configured.");
				return false;
			}

			const otpFormAction = extractFormAction(loginHtml);
			if (!otpFormAction) {
				adapter.log.warn("[Web] TOTP form found but no action URL.");
				return false;
			}
