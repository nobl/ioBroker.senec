...
/**
 * A mein-senec.de response that was not a success.
 *
 * The status is carried as httpStatus rather than as the axios-shaped `response.status`, so
 * that AdaptiveRequestQueue does not classify it a second time — rate limiting is already
 * reported to the queue by noteRateLimited before this is thrown.
 */
class WebHttpError extends Error {
	/**
	 * @param {string} label - HTTP method label, for the message
	 * @param {string} url - Requested URL
	 * @param {object} res - The axios response
	 */
	constructor(label, url, res) {
		super(`mein-senec.de ${label} ${url} failed with HTTP ${res?.status}`);
...
/**
 * Whether a status means the portal is refusing work rather than answering it.
 *
 * Shared by the retry predicate and the queue's success check so the two cannot drift: a
 * status that pauses the queue must also not be counted as a successful request.
 *
 * @param {number} [status] - HTTP status, if there is one
 * @returns {boolean} True for rate limiting and server-side errors
 */
...
/**
 * Whether a failed mein-senec.de request is worth another attempt.
 *
 * Rate limiting and server-side errors say something about the moment; a 4xx says the
 * request itself cannot succeed. A failure with no status at all is a transport problem
 * (timeout, reset), which is the case retrying was built for.
 *
 * @param {Error & { httpStatus?: number }} err - The error thrown by the request
 * @returns {boolean} True when a retry could plausibly succeed
 */
function isTransientWebError(err) {
	const status = err?.httpStatus;
	if (typeof status === "number") {
		return isOverloadStatus(status);
	}
	return true;
}
 ...
 * @param {{ maxRetries?: number, label?: string, timeout?: number, rawStatus?: boolean }} [queueOptions] - Queue retry options, an optional per-request timeout, and rawStatus to receive non-2xx responses instead of having them raised
...
async function _webRequest(adapter, method, url, data, queueOptions) {
	...
	const doRequest = async () => {
		...
		// authClient's default suits the short JSON calls the portal is mostly made of.
		// A caller that knows its response is larger says so rather than raising it for all.
		const baseConfig = {
			jar: adapter.webJar || undefined,
			maxRedirects: 5,
			...(queueOptions?.timeout ? { timeout: queueOptions.timeout } : {}),
		};
		...
		const send = () => (method === "get" ? client.get(url, config) : client.post(url, data, config));

		/**
		 * Log a response, report rate limiting, and recover an expired session.
		 *
		 * Both attempts go through here. The retry after a re-login used to be returned
		 * straight to the caller, which skipped this handling entirely — and it is the most
		 * likely one to be throttled, being a login and a request in quick succession.
		 *
		 * @param {object} res - The axios response to handle
		 * @param {boolean} allowReauth - False on the retry, so a login page cannot loop
		 * @returns {Promise<object>} The response to hand back to the caller
		 */
		const handle = async (res, allowReauth) => {
			if (adapter.config.web_reqnresp_log) {
				adapter.log.debug(
			...
			// validateStatus above means a 429 or a 503 arrives as a perfectly ordinary
			// response, so the queue would score it as a success and keep the pressure up.
			// Hand it over instead: the cooldown it starts holds back every queued portal
			// request, not just this one, which is the behaviour a struggling server needs.
			const overload = adapter.webQueue?.noteOverloadResponse(res.status, res.headers?.["retry-after"]);
			if (overload === "rate-limited") {
				adapter.log.warn("[Web] 🚦 429 (rate limited) — backing off.");
			} else if (overload === "server-error") {
				adapter.log.warn(`[Web] 🚦 HTTP ${res.status} from the portal — backing off.`);
			}

			// An expired session shows up two ways: the portal serves its login page with a
			// cheerful HTTP 200, or it answers the API call with 401/403. Both mean the same
			// thing, and both get exactly one recovery attempt — allowReauth is false on the
			// retry, so a portal that keeps refusing cannot drive a login loop.
			const sessionExpired =
				(res.status === 200 && typeof res.data === "string" && res.data.includes("Login - SENEC")) ||
				res.status === 401 ||
				res.status === 403;

			if (allowReauth && sessionExpired) {
				adapter.log.debug("[Web] Session expired, re-authenticating...");
				// Called through module.exports so the recovery path can be driven in tests
				// without standing up the whole SSO flow; the login itself is covered
				// separately. A direct reference would silently ignore any substitution and
				// make a re-authentication test pass without ever exercising one.
				adapter.webAuthenticated = await module.exports.webLogin(adapter);
				if (!adapter.webAuthenticated) {
					throw new Error("mein-senec.de re-authentication failed");
				}
				return handle(await send(), false);
			}
			return res;
		};

		const res = await handle(await send(), true);

		// Central status policy. validateStatus above is what lets the control call sites read
		// the portal's own error body, but it also means every other caller would have to
		// remember to check res.status — and a forgotten check writes an error page into
		// states as if it were data. Raise here so forgetting is not possible; callers that
		// genuinely interpret status ask for it with rawStatus.
		if (!queueOptions?.rawStatus && (res.status < 200 || res.status >= 300)) {
			throw new WebHttpError(label, url, res);
		}
		return res;
	};
	...
	if (adapter.webQueue) {
		return adapter.webQueue.add(doRequest, {
			...queueOptions,
			shouldRetry: isTransientWebError,
			// rawStatus callers get overload responses handed back instead of raised, so the
			// queue would otherwise score them as successes. It must not matter to the queue's
			// accounting whether a caller reads the status itself.
			isSuccess: (res) => !isOverloadStatus(res?.status),
		});
	}
	return doRequest();
}
 ...
 * @param {SenecAdapter} adapter - Senec adapter instance
 * @param {string} url - URL to request
 * @param {{ maxRetries?: number, label?: string, timeout?: number }} [queueOptions] - Queue retry options, plus an optional per-request timeout
 * @returns {Promise<object>} axios response
 */
 ...
 * @param {{ maxRetries?: number, label?: string, timeout?: number, rawStatus?: boolean }} [queueOptions] - Queue retry options, plus rawStatus for callers that report the portal's own error status
 * @returns {Promise<object>} axios response
 */
async function webPost(adapter, url, data, queueOptions) {
	return _webRequest(adapter, "post", url, data, queueOptions);
}
...
async function webPoll(adapter) {
	if (adapter.unloaded || adapter.webMasterPlantNumber === null) {
		return;
	}
	...
	try {
		adapter.log[adapter.config.web_showPolling ? "info" : "debug"]("[Web] 🔄 Polling mein-senec.de...");
		...
		try {
			...
			if (res?.data && typeof res.data === "object") {
				...
				if (adapter.config.web_measurements) {
					// SOC has no history endpoint — sample it here, see webRecordSoc()
					await webRecordSoc(adapter, res.data);
				}
				await adapter.updateLastPoll("_meinsenec.info.lastPoll.Status", "Last status poll");
			}
			if (!adapter.webConnected) {
				adapter.webConnected = true;
				await adapter.updateConnectionStatus();
				adapter.log.info("[Web] ✅ Connection restored.");
			}
		} catch (error) {
	...
	// Queue diagnostics, once per poll cycle. These used to sit at the end of
	// webPollMeasurements, which meant two independent debug settings silently did nothing
	// unless measurement polling happened to be switched on as well.
	if (adapter.config.web_debug_states) {
		try {
			await webUpdateQueueStats(adapter);
		} catch (error) {
			adapter.log.debug(`[Web] Failed to update queue stats: ${error.message}`);
		}
	}

	if (adapter.config.web_debug_log) {
		try {
			logWebQueueRecommendationIfChanged(adapter);
			logWebQueueStatsIfChanged(adapter);
		} catch (error) {
			adapter.log.debug(`[Web] Failed to log queue stats: ${error.message}`);
		}
	}
	...
	if (!adapter.unloaded) {
		...
		adapter.log[adapter.config.web_showPolling ? "info" : "debug"](
			`[Web] ⏱ Next mein-senec.de poll in ${(adapter.webStatusIntervalMs / 1000).toFixed(0)}s`,
		);
	}
}
...
async function webHandleControl(adapter, subId, state) {
	...
	if (subId === "EmergencyPower.ReserveInPercent") {
		...
		try {
			// rawStatus: this call reports the portal's own refusal message to the user,
			// which needs the failed response rather than an exception.
...
async function webLogin(adapter) {
	...
	try {
		...
		// Action and fields from one match: the credentials must be posted to the action of the very
		// form whose hidden fields go with them, and Keycloak carries the state of the flow in those.
		const loginForm = matchLoginForm(html);
		...
		if (!loginForm) {
			...
			// This is the unexpected-route case, so the page is exactly what a report needs — and
			// exactly the page that carries the session code and the account address.
			adapter.log.debug(`[Web] 🔍 Login page: ${redactAuthHtml(html, { mail: email, password })}`);
			return false;
		}
		...
		const loginRes = await client.post(
			loginForm.action,
			buildLoginForm(loginForm.inner, { username: email, password: password }).toString(),
			{
				jar: jar,
				maxRedirects: 10,
				validateStatus: () => true,
		...
		if (hasOtp(loginHtml)) {
			...
			const otpForm = matchLoginForm(loginHtml);
			if (!otpForm) {
			...
			const otpRes = await client.post(
				otpForm.action,
				buildLoginForm(otpForm.inner, { otp: totpCode }).toString(),
				{
					jar: jar,
					maxRedirects: 10,
					validateStatus: () => true,
...
// ── Statistical data (weekly CSV) ────────────────────────────────────────────

/** Base path for the statistics week index */
const STATS_PFX = "_meinsenec.Statistics.";

/**
 * Timeout for a statistics week download.
 *
 * A week at 5-minute resolution is far larger than the portal's usual JSON replies, so the
 * short default does not fit. Kept under the 60 s the dashboard waits so that a slow server
 * normally surfaces as a real error rather than as the dashboard's own generic timeout —
 * queue time is not counted here, so a request delayed behind a cooldown can still outlast it.
 */
const STATISTICS_TIMEOUT_MS = 45000;

/**
 * Refresh the list of downloadable statistics weeks, for every plant on the account.
 *
 * Only the index is stored, never the files: a week list is a few KB, while the CSVs
 * behind it run to tens of megabytes. The dashboard reads this to build its picker and
 * fetches a single week on demand through the statsCsv message.
 *
 * @param {SenecAdapter} adapter - Senec adapter instance
 * @returns {Promise<void>}
 */
async function webPollStatisticsWeeks(adapter) {
	const res = await webGet(adapter, `${WEB_HOST}/endkunde/api/anlagen/getAnlagen`);
	if (!Array.isArray(res?.data)) {
		adapter.log.debug("[Web] ⚠️ getAnlagen returned no plant list, skipping statistics index");
		return;
	}

	for (const plant of res.data) {
		const pn = plant?.anlageNummer;
		if (typeof pn !== "number") {
			continue;
		}
		const pfx = `${STATS_PFX}${pn}.`;
		try {
			const weeksRes = await webGet(
				adapter,
				`${WEB_HOST}/endkunde/api/statistischeDaten/getKalenderWochen?anlageNummer=${pn}`,
			);
			if (!Array.isArray(weeksRes?.data)) {
				continue;
			}
			// Newest first from the portal; keep that order for the picker
			const weeks = weeksRes.data
				.filter((w) => w && typeof w.kalenderwoche === "number" && typeof w.jahr === "number")
				.map((w) => ({ jahr: w.jahr, kw: w.kalenderwoche, von: w.starttag, bis: w.endtag }));

			await adapter.doState(`${pfx}name`, String(plant.produktName || `Plant ${pn}`), "Product Name", "", false);
			await adapter.doState(
				`${pfx}deviceNumber`,
				String(plant.steuereinheitnummer || ""),
				"Device Number",
				"",
				false,
			);
			// The plant this instance polls — the dashboard preselects it
			await adapter.doState(
				`${pfx}polled`,
				pn === adapter.webMasterPlantNumber,
				"Polled by this instance",
				"",
				false,
			);
			await adapter.doState(`${pfx}weeks`, JSON.stringify(weeks), "Downloadable weeks", "", false);
			adapter.log.debug(`[Web] Statistics index: plant ${pn} has ${weeks.length} weeks`);
		} catch (error) {
			adapter.logError(error, `[Web] ❌ Statistics week list failed for plant ${pn}`);
		}
	}
	await adapter.updateLastPoll("_meinsenec.info.lastPoll.Statistics", "Last statistics index poll");
}

/**
 * Download one statistics week and return its rows.
 *
 * Nothing is stored — the result goes straight back to the caller. The adapter is only
 * lending its portal session to a dashboard that has no way to authenticate itself.
 *
 * @param {SenecAdapter} adapter - Senec adapter instance
 * @param {number} pn - Plant number (anlageNummer)
 * @param {number} week - Calendar week
 * @param {number} year - Year
 * @returns {Promise<{ header: string[], rows: Array<Array<number|null>>, ts: number[] }>} Parsed week
 */
async function webFetchStatisticsWeek(adapter, pn, week, year) {
	const url = `${WEB_HOST}/endkunde/api/statistischeDaten/download?anlageNummer=${pn}&woche=${week}&jahr=${year}`;
	const res = await webGet(adapter, url, {
		label: `statistics ${year}-KW${week}`,
		timeout: STATISTICS_TIMEOUT_MS,
	});
	// An error page is a string too, and the CSV parser finds no timestamped rows in it — so
	// without this the week would quietly render as "no data" instead of as the refusal it is.
	if (res?.status !== 200) {
		throw new Error(`Statistics download for ${year}-KW${week} returned HTTP ${res?.status}`);
	}
	if (typeof res.data !== "string") {
		throw new Error(`Unexpected statistics response for ${year}-KW${week}`);
	}
	return parseStatisticsCsv(res.data);
}

/**
 * Parse a statistics CSV: semicolon separated, German decimal commas, dd.mm.yyyy HH:MM:SS.
 *
 * The final row wraps back to the start of the week, so rows are sorted by timestamp
 * rather than trusted in file order.
 *
 * @param {string} text - Raw CSV
 * @returns {{ header: string[], rows: Array<Array<number|null>>, ts: number[] }} Parsed week
 */
function parseStatisticsCsv(text) {
	...
	for (const line of lines) {
		...
		const when = /^(\d{2})\.(\d{2})\.(\d{4}) (\d{2}):(\d{2}):(\d{2})$/.exec(parts[0].trim());
		if (!when) {
			if (!header.length && parts.length > 1) {
				header = parts.map((h) => h.trim());
			}
			continue;
		}
		const ts = new Date(+when[3], +when[2] - 1, +when[1], +when[4], +when[5], +when[6]).getTime();
		const values = parts.slice(1).map((v) => {
			const n = v.trim() === "" ? NaN : Number(v.replace(",", "."));
			return Number.isFinite(n) ? n : null;
		});
		parsed.push({ ts, values });
	}

	parsed.sort((a, b) => a.ts - b.ts);
	return { header, ts: parsed.map((p) => p.ts), rows: parsed.map((p) => p.values) };
}

// ── Battery SOC history ──────────────────────────────────────────────────────

/** Measurement prefixes the sampled SOC series is written under */
const SOC_TODAY_PFX = "_meinsenec.Measurements.Daily.today.";
const SOC_YESTERDAY_PFX = "_meinsenec.Measurements.Daily.yesterday.";
const SOC_CUR_MONTH_PFX = "_meinsenec.Measurements.Monthly.current_month.";
const SOC_PREV_MONTH_PFX = "_meinsenec.Measurements.Monthly.previous_month.";

/**
 * Local calendar tag "YYYY-MM-DD" for a date.
 *
 * @param {Date} d - Date to format
 * @returns {string} Day tag
 */
function socDayTag(d) {
	return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}-${String(d.getDate()).padStart(2, "0")}`;
}

/**
 * Write one sampled SOC value through evalPoll, so name and unit come from state_attr
 * ("acculevel.hourly" / ".daily" / ".mean" / ".day" / ".month") like every other state.
 *
 * @param {SenecAdapter} adapter - Senec adapter instance
 * @param {string} pfx - Measurement prefix (e.g. SOC_TODAY_PFX)
 * @param {string} keyPrefix - Key below the prefix, e.g. "acculevel.hourly"
 * @param {Record<string, string|number>} values - Values keyed by leaf name
 * @returns {Promise<void>}
 */
async function webSocWrite(adapter, pfx, keyPrefix, values) {
	await adapter.evalPoll(values, pfx, keyPrefix);
}

/**
 * Clear a calculated SOC state at a day or month rollover.
 *
 * Written straight to the state rather than through evalPoll/doState, for two reasons:
 * ValueTyping() turns null into 0 for any key carrying a unit, so an hour that was never
 * sampled would read as a real 0%; and doState() derives common.type from the value, so
 * clearing would rewrite the object to "mixed" and the next sample back to "number".
 * No metadata is needed here because only states that were previously written get cleared —
 * the object and its state_attr-derived name and unit already exist.
 *
 * @param {SenecAdapter} adapter - Senec adapter instance
 * @param {string} id - Full state id
 * @returns {Promise<void>}
 */
async function webSocClear(adapter, id) {
	await adapter.setStateAsync(id, null, true);
}

/**
 * Sample the battery state of charge into the measurement tree.
 *
 * mein-senec offers no SOC history: getstatus.php rejects type=acculevel, and the
 * voltage/current series from getaccustate.php cannot be integrated into one, because
 * the capacity that endpoint reports does not reconcile with the observed energy
 * throughput (reconstructing a real day against the reported 10 kWh drives the curve
 * to -9%, and no single capacity satisfies both that floor and the reported daily
 * mean). So the series is sampled instead: every status poll feeds the running mean of
 * the current hour, and at midnight the finished day is frozen into the monthly series.
 *
 * Consequence worth knowing: values exist only for the time the adapter is running. A
 * day it was down over midnight loses its average for good — nothing can fetch it back.
 *
 * @param {SenecAdapter} adapter - Senec adapter instance
 * @param {{ acculevel?: { now?: number | string, today?: number | string } }} status - getstatusoverview.php response
 * @returns {Promise<void>}
 */
async function webRecordSoc(adapter, status) {
	const soc = Number(status?.acculevel?.now);
	if (!isFinite(soc) || soc < 0 || soc > 100) {
		return;
	}

	const now = new Date();
	const dayTag = socDayTag(now);
	const hour = now.getHours();

	// The running day's 24 hourly means are held in memory and only read back from state
	// once per process. This runs on the fast status tier, so re-reading them every poll
	// just to recompute the day mean would cost 24 state reads per cycle.
	if (adapter._webSocHours === undefined) {
		adapter._webSocHours = [];
		let latestTs = 0;
		for (let h = 0; h < 24; h++) {
			const st = await adapter.getStateAsync(`${SOC_TODAY_PFX}acculevel.hourly.${h}`);
			const val = st?.val != null && isFinite(Number(st.val)) ? Number(st.val) : null;
			adapter._webSocHours.push(val);
			if (val !== null && st?.ts && st.ts > latestTs) {
				latestTs = st.ts;
			}
		}
		// Which day those hours belong to, taken from when they were last written rather than
		// from a marker state. Every rollover clears the hours to null, so the first write of a
		// day is always a change and carries a timestamp inside that day.
		adapter._webSocDay = latestTs ? socDayTag(new Date(latestTs)) : null;
		// The running day mean, so a rollover right after a restart can still freeze it
		const storedMean = await adapter.getStateAsync(`${SOC_TODAY_PFX}acculevel`);
		adapter._webSocDayMean =
			storedMean?.val != null && isFinite(Number(storedMean.val)) ? Number(storedMean.val) : null;
	}

	if (adapter._webSocDay !== dayTag) {
		if (adapter._webSocDay) {
			await webRollOverSoc(adapter, adapter._webSocDay, dayTag);
		}
		adapter._webSocDay = dayTag;
		adapter._webSocBucket = null;
	}

	const hours = adapter._webSocHours;

	// Restarting mid-hour must not discard the mean already accumulated for that hour
	let bucket = adapter._webSocBucket;
	if (!bucket || bucket.day !== dayTag || bucket.hour !== hour) {
		bucket = { day: dayTag, hour, sum: 0, count: 0 };
		if (hours[hour] != null) {
			bucket.sum = hours[hour];
			bucket.count = 1;
		}
		adapter._webSocBucket = bucket;
	}

	bucket.sum += soc;
	bucket.count++;
	hours[hour] = Math.round((bucket.sum / bucket.count) * 10) / 10;

	await webSocWrite(adapter, SOC_TODAY_PFX, "acculevel.hourly", { [hour]: hours[hour] });

	// The day mean comes from the portal, not from our samples: acculevel.today covers the
	// whole day including the hours before the adapter was running, so it stays correct after
	// a late start where a mean over our own samples would be biased to the sampled window.
	const dayMean = Number(status?.acculevel?.today);
	if (isFinite(dayMean) && dayMean >= 0 && dayMean <= 100) {
		adapter._webSocDayMean = Math.round(dayMean * 10) / 10;
		await webSocWrite(adapter, SOC_TODAY_PFX, "", { acculevel: adapter._webSocDayMean });
	}
}

/**
 * Move a finished day out of the "today" slot: into "yesterday", and its mean into the
 * monthly series. Migrates the monthly series to previous_month when the month changed.
 *
 * @param {SenecAdapter} adapter - Senec adapter instance
 * @param {string} endedDayTag - Day tag ("YYYY-MM-DD") of the day that just finished
 * @param {string} newDayTag - Day tag ("YYYY-MM-DD") of the day now starting
 * @returns {Promise<void>}
 */
async function webRollOverSoc(adapter, endedDayTag, newDayTag) {
	const [year, month, day] = endedDayTag.split("-").map(Number);
	if (!year || !month || !day) {
		return;
	}
	adapter.log.debug(`[Web] 🔄 Freezing sampled SOC for ${endedDayTag}`);

	// The "yesterday" slot is only meaningful when the finished day really is the day
	// before. After a gap (adapter down) it must be cleared, not filled with stale data —
	// the chart labels that slot "yesterday" without checking its date.
	//
	// Adjacency is a calendar question, not a duration one: the night Europe/Berlin springs
	// forward is 23 hours and the night it falls back is 25, so comparing elapsed
	// milliseconds against 86400000 declared those two days non-adjacent and threw away a
	// complete day of samples twice a year. Stepping the calendar date instead is exact, and
	// still rejects a genuine multi-day gap.
	const isAdjacent = socDayTag(new Date(year, month - 1, day + 1)) === newDayTag;
	const hours = adapter._webSocHours || [];
	const dayMean = adapter._webSocDayMean ?? null;

	for (let h = 0; h < 24; h++) {
		const carried = isAdjacent ? (hours[h] ?? null) : null;
		if (carried !== null) {
			await webSocWrite(adapter, SOC_YESTERDAY_PFX, "acculevel.hourly", { [h]: carried });
		} else {
			// Only states that already hold a value need clearing
			const st = await adapter.getStateAsync(`${SOC_YESTERDAY_PFX}acculevel.hourly.${h}`);
			if (st?.val != null) {
				await webSocClear(adapter, `${SOC_YESTERDAY_PFX}acculevel.hourly.${h}`);
			}
		}
		if (hours[h] !== null) {
			await webSocClear(adapter, `${SOC_TODAY_PFX}acculevel.hourly.${h}`);
			hours[h] = null;
		}
	}
	if (isAdjacent && dayMean !== null) {
		await webSocWrite(adapter, SOC_YESTERDAY_PFX, "", { acculevel: dayMean });
	} else {
		await webSocClear(adapter, `${SOC_YESTERDAY_PFX}acculevel`);
	}
	if (dayMean !== null) {
		await webSocClear(adapter, `${SOC_TODAY_PFX}acculevel`);
	}
	adapter._webSocDayMean = null;

	// A stale day — the adapter was down across one or more midnights — is not filed: its
	// mean covers only part of that day, and filing it would also break the month inference
	// below, which reads a daily value's own age to decide which month it belongs to.
	if (dayMean !== null && isAdjacent) {
		// Store the finished day in the month it belongs to, then — if the new day opens a
		// different month — shift that month down to previous_month. Both steps are needed:
		// on 1 August the 31 July value still belongs to current_month, and only afterwards
		// does July become the previous month.
		await webSocEnsureMonth(adapter, `${year}-${String(month).padStart(2, "0")}`);
		await webSocWrite(adapter, SOC_CUR_MONTH_PFX, "acculevel.daily", { [day]: dayMean });
		await webSocUpdateMonthMean(adapter, SOC_CUR_MONTH_PFX);
	}
	await webSocEnsureMonth(adapter, newDayTag.slice(0, 7));
}

/**
 * Which month the values under current_month describe, derived from the values themselves.
 *
 * A daily mean is written just after the midnight that ends the day it describes, so that day
 * is the write timestamp minus 24 hours. The inference is only accepted when the day-of-month
 * it yields matches the index the value is stored under — anything else was filed late and
 * cannot be trusted to name the month.
 *
 * @param {SenecAdapter} adapter - Senec adapter instance
 * @returns {Promise<string|null>} Month tag ("YYYY-MM"), or null when nothing is stored
 */
async function webSocCurrentMonthTag(adapter) {
	let bestTs = 0;
	let tag = null;
	for (let d = 1; d <= 31; d++) {
		const st = await adapter.getStateAsync(`${SOC_CUR_MONTH_PFX}acculevel.daily.${d}`);
		if (st?.val == null || !isFinite(Number(st.val)) || !st.ts) {
			continue;
		}
		// The value describes the calendar day before the one it was written on. Stepping
		// back 24 hours instead lands on the wrong date when the day in between was not 24
		// hours long: a value written just after midnight on 30 March 2026 in Europe/Berlin
		// resolved to 28 March, failed the day-of-month check below, and left the month
		// unidentifiable.
		const writtenAt = new Date(st.ts);
		const described = new Date(writtenAt.getFullYear(), writtenAt.getMonth(), writtenAt.getDate() - 1);
		if (described.getDate() !== d || st.ts <= bestTs) {
			continue;
		}
		bestTs = st.ts;
		tag = `${described.getFullYear()}-${String(described.getMonth() + 1).padStart(2, "0")}`;
	}
	return tag;
}

/**
 * Refresh the month-level mean from the daily means stored under it, so the node carries a
 * value like its siblings (consumption, powergenerated, ...) rather than being a bare folder.
 *
 * @param {SenecAdapter} adapter - Senec adapter instance
 * @param {string} pfx - Month prefix (current or previous)
 * @returns {Promise<void>}
 */
async function webSocUpdateMonthMean(adapter, pfx) {
	let sum = 0;
	let count = 0;
	for (let d = 1; d <= 31; d++) {
		const st = await adapter.getStateAsync(`${pfx}acculevel.daily.${d}`);
		if (st?.val != null && isFinite(Number(st.val))) {
			sum += Number(st.val);
			count++;
		}
	}
	if (count) {
		await webSocWrite(adapter, pfx, "", { acculevel: Math.round((sum / count) * 10) / 10 });
	} else {
		await webSocClear(adapter, `${pfx}acculevel`);
	}
}

/**
 * Make current_month hold the given month, moving whatever it held before to
 * previous_month. No-op when it already holds that month.
 *
 * @param {SenecAdapter} adapter - Senec adapter instance
 * @param {string} monthTag - Target month ("YYYY-MM")
 * @returns {Promise<void>}
 */
async function webSocEnsureMonth(adapter, monthTag) {
	const curMonth = await webSocCurrentMonthTag(adapter);
	if (curMonth === null || curMonth === monthTag) {
		return;
	}
	{
		for (let d = 1; d <= 31; d++) {
			...
			const val = st?.val != null && isFinite(Number(st.val)) ? Number(st.val) : null;
			if (val !== null) {
				await webSocWrite(adapter, SOC_PREV_MONTH_PFX, "acculevel.daily", { [d]: val });
				await webSocClear(adapter, `${SOC_CUR_MONTH_PFX}acculevel.daily.${d}`);
			} else {
				const prev = await adapter.getStateAsync(`${SOC_PREV_MONTH_PFX}acculevel.daily.${d}`);
				if (prev?.val != null) {
					await webSocClear(adapter, `${SOC_PREV_MONTH_PFX}acculevel.daily.${d}`);
				}
			}
		}
		await webSocUpdateMonthMean(adapter, SOC_PREV_MONTH_PFX);
		await webSocUpdateMonthMean(adapter, SOC_CUR_MONTH_PFX);
	}
}
...
async function webUpdateQueueStats(adapter) {
	...
	// The state IDs are kept as they are so existing history stays continuous; only the
	// fields they read are corrected. getStats() publishes queued/running/started, never the
	// pending/active/total/successRate these used to ask for — so four of the six states had
	// been written as undefined ever since they were added.
	//
	// "Processed" means finished, so it counts outcomes rather than starts: `started` also
	// includes whatever is in flight, and the web connector routinely has work in flight
	// because secondary-plant polls are started without being awaited. Using it would
	// overstate the total and drag the success rate down with unfinished requests.
	const completed = (stats.succeeded || 0) + (stats.failed || 0);
	const successRate = completed > 0 ? Number(((stats.succeeded / completed) * 100).toFixed(2)) : 100;

	await adapter.doState(`${pfx}currentConcurrency`, stats.concurrency, "Current queue concurrency", "", false);
	await adapter.doState(`${pfx}pendingRequests`, stats.queued, "Pending requests", "", false);
	await adapter.doState(`${pfx}activeRequests`, stats.running, "Active requests", "", false);
	await adapter.doState(`${pfx}totalRequests`, completed, "Total requests processed", "", false);
	await adapter.doState(`${pfx}successRate`, successRate, "Success rate", "%", false);
	await adapter.doState(
 ...
 * Info logging is only emitted when web_debug_log is enabled.
 *
 * @param {SenecAdapter} adapter - Senec adapter instance
 * @returns {void}
 */
function logWebQueueRecommendationIfChanged(adapter) {
	if (!adapter.webQueue || typeof adapter.webQueue.getStats !== "function") {
		return;
	}
	...
	if (adapter._lastLoggedWebRecommendedConcurrency !== stats.recommendedConcurrency) {
		adapter._lastLoggedWebRecommendedConcurrency = stats.recommendedConcurrency;

		adapter.log.info(
			`[Web] AdaptiveRequestQueue recommends concurrency=${stats.recommendedConcurrency} ` +
...
/**
 * Log current web queue statistics only if key values changed since the last log.
 * Info logging is only emitted when web_debug_log is enabled.
 *
 * @param {SenecAdapter} adapter - Senec adapter instance
 * @returns {void}
 */
function logWebQueueStatsIfChanged(adapter) {
	if (!adapter.webQueue || typeof adapter.webQueue.getStats !== "function") {
		return;
	}
	...
	if (adapter._lastLoggedWebQueueSnapshot !== snapshot) {
		adapter._lastLoggedWebQueueSnapshot = snapshot;

		adapter.log.info(
			`[Web] Queue stats: current=${stats.concurrency}, recommended=${stats.recommendedConcurrency}, ` +
...
async function webPollMeasurements(adapter, pn) {
	...
	if (
		...
		try {
			...
			await webPollStatisticsWeeks(adapter);
	...
	for (const id of Object.keys(objects)) {
		if (id.startsWith(fullPrefix)) {
			try {
				...
				// doState caches every object it has seen and skips creating one it believes
				// already exists. The day-rollover callers delete and then rewrite the same ids
				// within one poll, so a cache entry left behind here means the state is written
				// back without its object — no name, no unit, no role.
				adapter.knownObjects.delete(id.slice(adapter.namespace.length + 1));
				count++;
			} catch {
				// ignore
			}
...
async function webStartupCleanup(adapter) {
	// Clean up queue diagnostics if debug states are disabled (per connector)
	if (!adapter.config.web_debug_states) {
...
/**
 * Extract a PEM file from a small ZIP buffer using raw ZIP format parsing.
 * Supports stored (method 0) and deflated (method 8) entries.
 * No external dependencies — uses Node's built-in zlib for decompression.
 *
 * @param {Buffer} zipBuffer - ZIP file contents
 * @returns {string | null} PEM string or null if not found
 */
function extractPemFromZip(zipBuffer) {
	...
	while (offset + 30 <= zipBuffer.length) {
		...
		if (fileName.endsWith(".pem") && dataSize > 0) {
			const rawData = zipBuffer.subarray(dataStart, dataStart + dataSize);
			if (compressionMethod === 0) {
				// Stored — read directly
				return rawData.toString("utf-8");
			} else if (compressionMethod === 8) {
				// Deflated — decompress with zlib.inflateRawSync
				try {
...
/**
 * Download the SenecGui-Root CA certificate from mein-senec.de documents API.
 * Searches global documents for the "SenecGui-Root" entry, downloads the ZIP,
 * and extracts the PEM.
 *
 * @param {SenecAdapter} adapter - Senec adapter instance
 * @returns {Promise<string | null>} PEM string or null if not found/failed
 */
async function webFetchCaCert(adapter) {
	if (!adapter.webConnected || !adapter.authClient) {
		return null;
	}

	// Step 1: Get global documents list
	const listRes = await webGet(adapter, `${WEB_HOST}/endkunde/api/dokumente/getGlobalDocuments`);
	if (!listRes?.data || !Array.isArray(listRes.data)) {
		adapter.log.debug("[Web] Could not fetch global documents list.");
		return null;
	}

	// Step 2: Find SenecGui-Root entry
	const certDoc = listRes.data.find(
		(doc) => doc.description === "SenecGui-Root" && doc.fileName && doc.documentPointer?.docId,
	);
	if (!certDoc) {
		adapter.log.debug("[Web] SenecGui-Root not found in global documents.");
		return null;
	}

	// Step 3: Download the ZIP (binary)
	const docId = certDoc.documentPointer.docId;
	adapter.log.debug(`[Web] Downloading SenecGui-Root cert (docId: ${docId})...`);

	const client = adapter.authClient;
	const dlRes = await client.get(`${WEB_HOST}/endkunde/api/dokumente/download?docId=${docId}`, {
		jar: adapter.webJar || undefined,
	...
	if (dlRes.status !== 200 || !dlRes.data) {
		adapter.log.debug(`[Web] Cert download returned HTTP ${dlRes.status}`);
		return null;
	}

	// Step 4: Extract PEM from ZIP
	const pem = extractPemFromZip(Buffer.from(dlRes.data));
	if (!pem || !pem.includes("BEGIN CERTIFICATE")) {
		adapter.log.debug("[Web] Could not extract PEM from downloaded ZIP.");
		return null;
	}

	adapter.log.info(`[Web] Successfully downloaded SenecGui-Root CA cert (${pem.length} bytes).`);
	return pem;
}
...
module.exports = {
	...
	webSocCurrentMonthTag,
	webPollStatisticsWeeks,
	webFetchStatisticsWeek,
	parseStatisticsCsv,
	webCleanupStates,
	webStartupCleanup,
	// Pure helpers
