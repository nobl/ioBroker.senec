...
async function _webRequest(adapter, method, url, data, queueOptions) {
	...
	const doRequest = async () => {
		...
		const handle = async (res, allowReauth) => {
			...
			// validateStatus above means a 429 arrives as a perfectly ordinary response, so
			// the queue would score it as a success and keep the pressure up. Hand it over
			// instead — this is what makes the cooldown work while browsing weeks quickly.
			if (adapter.webQueue?.noteRateLimited(res.status, res.headers?.["retry-after"])) {
...
async function webRollOverSoc(adapter, endedDayTag, newDayTag) {
	...
	const isAdjacent =
		new Date(`${newDayTag}T00:00:00`).getTime() - new Date(`${endedDayTag}T00:00:00`).getTime() === 86400000;
	const hours = adapter._webSocHours || [];
...
async function webSocCurrentMonthTag(adapter) {
	...
	for (let d = 1; d <= 31; d++) {
		...
		const described = new Date(st.ts - 86400000);
