...
async function _webRequest(adapter, method, url, data) {
	...
	const doRequest = async () => {
		...
		if (adapter.config.api_reqnresp_log) {
			adapter.log.debug(`[Web] ${label} response: HTTP ${res.status} → ${JSON.stringify(res.data)}`);
		} else {
 ...
 */
async function webFetchMeasurement(adapter, pn, type, period, params = {}) {
...
/**
 * Aggregate ~5-min interval data into hourly sums.
 *
 * @param {Array<[number, number]>} val - Array of [timestamp_ms, value_kwh]
 * @returns {Record<number, number>} Hourly sums keyed 0-23
 */
...
async function webWriteMeasurement(adapter, pfx, type, data, aggregations = {}) {
	// Write total
	await adapter.doState(`${pfx}${type}`, data.fullkwh, `${type} total`, "kWh", false);

	if (aggregations.hourly && data.val) {
		const hourly = aggregateToHourly(data.val);
		for (const [hour, value] of Object.entries(hourly)) {
			await adapter.doState(`${pfx}${type}.hourly.${hour}`, value, `${type} hour ${hour}`, "kWh", false);
		}
	}

	if (aggregations.detail && data.val) {
		for (let i = 0; i < data.val.length; i++) {
			await adapter.doState(`${pfx}${type}.detail.${i}`, data.val[i][1], `${type} detail ${i}`, "kWh", false);
		}
	}
	...
	if (aggregations.daily && data.val) {
		...
		for (const [day, value] of Object.entries(daily)) {
			await adapter.doState(`${pfx}${type}.daily.${day}`, value, `${type} day ${day}`, "kWh", false);
		}
	}
	...
	if (aggregations.monthly && data.val) {
		...
		for (const [month, value] of Object.entries(monthly)) {
			await adapter.doState(`${pfx}${type}.monthly.${month}`, value, `${type} month ${month}`, "kWh", false);
		}
	}
	...
	if (aggregations.yearly && data.val) {
		for (const [ts, value] of data.val) {
			...
			await adapter.doState(`${pfx}${type}.${year}`, value, `${type} year ${year}`, "kWh", false);
		}
	}
}
 ...
 */
async function webFetchAllTypes(adapter, pn, period, params = {}) {
	const results = await Promise.allSettled(
		WEB_MEASUREMENT_TYPES.map((type) => webFetchMeasurement(adapter, pn, type, period, params)),
	);
...
async function webPollMeasurementsToday(adapter, pn) {
	adapter.log.debug("[Web] 🔄 Polling measurements: today");
...
async function webPollMeasurementsYesterday(adapter, pn) {
	...
	adapter.log.debug("[Web] 🔄 Polling measurements: yesterday");
...
/**
 * Poll 7-day measurements (medium tier).
 * Writes fullkwh total and daily breakdown (0-7).
 *
 * @param {SenecAdapter} adapter - Senec adapter instance
 * @param {number} pn - Plant number
 * @returns {Promise<void>}
 */
async function webPollMeasurements7Days(adapter, pn) {
	adapter.log.debug("[Web] 🔄 Polling measurements: 7days");
	const pfx = "_meinsenec.Measurements.Daily.7days.";
	const data = await webFetchAllTypes(adapter, pn, "7days");
	for (const type of WEB_MEASUREMENT_TYPES) {
		if (data[type]) {
			await adapter.doState(`${pfx}${type}`, data[type].fullkwh, `${type} total`, "kWh", false);
			if (data[type].val) {
				for (let i = 0; i < data[type].val.length; i++) {
					await adapter.doState(`${pfx}${type}.${i}`, data[type].val[i][1], `${type} day ${i}`, "kWh", false);
				}
			}
		}
	}
	await adapter.doState(`${pfx}lastUpdated`, new Date().toISOString(), "Last updated", "", false);
	await adapter.updateLastPoll("_meinsenec.info.lastPoll.Measurements7Days", "Last 7-day measurements poll");
}
...
async function webPollMeasurementsMonth(adapter, pn, monthStart, label) {
	...
	adapter.log.debug(`[Web] 🔄 Polling measurements: ${label}`);
	const pfx = `_meinsenec.Measurements.Monthly.${label}.`;
	const data = await webFetchAllTypes(adapter, pn, "month", { timestamp: monthStart.getTime() });
	for (const type of WEB_MEASUREMENT_TYPES) {
		if (data[type]) {
...
/**
 * Poll yearly measurements (slow tier).
 * Uses period=range to get daily values for the entire year, then aggregates to monthly.
 *
 * @param {SenecAdapter} adapter - Senec adapter instance
 * @param {number} pn - Plant number
...
async function webPollMeasurementsYear(adapter, pn, year, label) {
	...
	const now = new Date();
	const endDate = dec31.getTime() > now.getTime() ? now : dec31;
	const days = Math.ceil((endDate.getTime() - jan1.getTime()) / (1000 * 60 * 60 * 24)) + 1;

	const pfx = `_meinsenec.Measurements.Yearly.${year}.`;
	const data = await webFetchAllTypes(adapter, pn, "range", { timestamp: jan1.getTime(), days });
	for (const type of WEB_MEASUREMENT_TYPES) {
		if (data[type]) {
			// Write yearly total
			await adapter.doState(`${pfx}${type}`, data[type].fullkwh, `${type} total ${year}`, "kWh", false);
			// Aggregate daily values to monthly
			if (data[type].val) {
				const monthly = aggregateToMonthly(data[type].val);
				for (const [month, value] of Object.entries(monthly)) {
					await adapter.doState(
...
async function webPollMeasurementsAllTime(adapter, pn) {
	// Skip if already updated this calendar year
	const lastUpdate = await adapter.getStateAsync("_meinsenec.Measurements.AllTime.lastUpdated");
	if (lastUpdate?.val) {
		const lastDate = new Date(String(lastUpdate.val));
		const now = new Date();
		if (!isNaN(lastDate.getTime()) && lastDate.getUTCFullYear() === now.getUTCFullYear()) {
			adapter.log.silly("[Web] AllTime measurements already updated this year. Skipping.");
			return;
		}
	}
...
async function webPollMeasurements(adapter, pn) {
	...
	// Status tier — today (every poll cycle)
	try {
		await webPollMeasurementsToday(adapter, pn);
	} catch (error) {
		adapter.logError(error, "[Web] ❌ Today measurements failed");
	}

	// Medium tier — yesterday, 7days, autarky
	if (
		...
		try {
			...
			await webPollMeasurements7Days(adapter, pn);
