...
async function webSocEnsureMonth(adapter, monthTag) {
	const stored = await adapter.getStateAsync(`${SOC_CUR_MONTH_PFX}acculevel.month`);
	const curMonth = stored?.val ? String(stored.val) : null;
	if (curMonth === monthTag) {
		return;
	}
	if (curMonth) {
		for (let d = 1; d <= 31; d++) {
		...
		await webSocWrite(adapter, SOC_PREV_MONTH_PFX, "acculevel", { month: curMonth });
		await webSocUpdateMonthMean(adapter, SOC_PREV_MONTH_PFX);
		await webSocUpdateMonthMean(adapter, SOC_CUR_MONTH_PFX);
	}
	await webSocWrite(adapter, SOC_CUR_MONTH_PFX, "acculevel", { month: monthTag });
}
