...
// Largest lala.cgi request body the appliance was observed to answer in full, in bytes. Measured
// against a SENEC.Home V3 in August 2026: 11428 bytes came back complete, 11998 bytes came back as
// HTTP 200 with a truncated, unparseable body. Model- and firmware-specific — an observation, not
// a documented spec, so treat it as an order of magnitude rather than an exact figure.
const LOCAL_REQUEST_SIZE_LIMIT = 11500;

// Warn at half the ceiling so an oversized form is reported while it still works, rather than once
// it has already started coming back truncated.
const LOCAL_REQUEST_SIZE_WARN_RATIO = 0.5;
...
async function localInitPollSettings(adapter) {
	...
	// On the size of these forms, because it looks like it wants chunking and it does not:
	//
	// The appliance's own web interface splits its lala.cgi posts into pieces of at most 600 bytes
	// — the n() function in senec.min.js fires a request whenever the next section would cross
	// that mark. That is a convention of that UI, not something the appliance imposes: the real
	// ceiling is LOCAL_REQUEST_SIZE_LIMIT, some nineteen times higher. Response size is no
	// constraint either — on that same appliance, bodies of 19762, 20753 and 22233 bytes all
	// parsed, which is why the low-priority form asks for every section at once. It runs around
	// 520 bytes and the high-priority one around 960, leaving them roughly twenty-two and twelve
	// times under the ceiling. Chunking would buy nothing, cost a round trip per piece, and break
	// the snapshot: split across several polls, grid and house power are read at different
	// instants and stop adding up.
	//
	// Both inputs that can grow a form — section discovery, which may extend the list below, and
	// addUserDps — validate names against [A-Z0-9_], but neither caps how many it accepts. Hence
	// the size check at the end of this function.
	...
	adapter.lowPrioForm = "{";
	// Discovery may have extended the shipped list for this device; it never shrinks it, and
	// it never edits the shared baseline. Falling back to the baseline covers the cases where
	// discovery has not run or could not complete.
	for (const value of adapter.knownSections || allKnownObjects) {
		...
		switch (value) {
			...
			case "ENERGY":
				[
					...
					"GUI_BAT_DATA_FUEL_CHARGE",
					// GUI_BAT_DATA_OA_CHARGING was polled here until 2026-08. A SENEC.Home V3
					// answers VARIABLE_NOT_FOUND for it, it is absent from all 38 fields that
					// appliance reports for this section, and its own web interface never asks
					// for it — so it is a field that has gone away. Asking every ten seconds
					// cost bytes and produced nothing. It is deliberately NOT removed from
					// state_attr: the low-priority poll requests the whole ENERGY section, so
					// an older appliance that still has the field keeps its state, just at the
					// slower interval.
	...
	warnIfFormOversized(adapter, "low-priority", adapter.lowPrioForm);
	warnIfFormOversized(adapter, "high-priority", adapter.highPrioForm);
}

/**
 * Warn when an assembled poll form approaches the measured request size ceiling.
 * Past that ceiling the appliance answers with HTTP 200 and a truncated body, which only surfaces
 * as a JSON parse error in localPoll and is then handled as a communication failure — so the size
 * is worth naming here, where it is still recognisable as a size problem. The form is not
 * rejected: the ceiling is measured rather than documented and differs by model and firmware.
 *
 * @param {SenecAdapter} adapter - Senec adapter instance
 * @param {string} label - Human-readable form name for the log message
 * @param {string} form - The assembled request form
 * @returns {void}
 */
function warnIfFormOversized(adapter, label, form) {
	const bytes = Buffer.byteLength(form, "utf8");
	const threshold = Math.round(LOCAL_REQUEST_SIZE_LIMIT * LOCAL_REQUEST_SIZE_WARN_RATIO);
	if (bytes <= threshold) {
		return;
	}
	adapter.log.warn(
		`[Local] ⚠️ The ${label} poll form is ${bytes} bytes, past the ${
			threshold
		} byte mark this adapter stays under (measured request ceiling ~${
			LOCAL_REQUEST_SIZE_LIMIT
		} bytes). Oversized requests come back as HTTP 200 with a truncated body, which shows up as a JSON parse error and a lost connection rather than as a size problem. Consider fewer high-priority datapoints, or fewer polled sections.`,
	);
}
...
function addUserDps(adapter, value, objectsSet, dpToAdd) {
	// Split first, then judge each name on its own. Validating the raw string instead would
	// reject "DP_1, DP_2" over the blank after the comma, and would let the empty token in
	// "DP_1,DP_2," through into the poll form as a nameless datapoint.
	const datapoints = String(dpToAdd ?? "")
		.toUpperCase()
		.split(",")
		.map((item) => item.trim())
		.filter((item) => item.length > 0);
	if (datapoints.length === 0) {
		// Nothing configured. The activation checkbox stays ticked when the field is cleared,
		// so this is a normal state rather than a misconfiguration worth warning about.
		return;
	}
	const invalid = datapoints.filter((item) => !/^[A-Z0-9_]+$/.test(item));
	if (invalid.length > 0) {
		// Reject the whole entry rather than polling the good half: requesting the wrong
		// datapoints can make the appliance misbehave, so a typo should poll nothing.
		adapter.log.warn(
			`[Local] ⚠️ Datapoints config for ${
				value
			} may only contain [A-Z0-9_], separated by commas - ignoring the whole entry. Offending: ${invalid.join(", ")}`,
		);
		return;
	}
	datapoints.forEach((item) => objectsSet.add(item));
	adapter.log.debug(`[Local] (addUserDps) Datapoints config changed for ${value}: ${datapoints.join(",")}`);
}
...
async function localDiscoverSections(adapter) {
	...
	try {
		...
		for (const group of ["DEBUG", "PLAIN"]) {
			if (data[group] && Array.isArray(data[group].SECTIONS)) {
				for (const entry of data[group].SECTIONS) {
					...
					// Same character rule addUserDps applies to user-supplied datapoint names, but
					// tested as received instead of upper-cased: these are literal JSON keys that
					// localInitPollSettings concatenates into the request form verbatim, so
					// normalising the case would change what we ask the device for. A name holding
					// a double quote breaks the form open ({"EV"IL":{}} does not parse), and every
					// section name the shipped list knows passes this rule. One malformed entry is
					// no reason to throw away the rest, so the skip is per name.
		...
		if (rejected.length > 0) {
			adapter.log.warn(
				`[Local] ⚠️ Device advertised ${
					rejected.length
				} section name(s) outside [A-Z0-9_] - ignoring them. Offending: ${rejected.join(", ")}`,
			);
		}

		// Sections the device advertises that the shipped list does not know about yet.
		const newSections = [];
		...
		// Shipped sections the device did not advertise. Computed against the baseline, which
		// is why it has to be read before anything is derived from it — the previous version
		// deleted these first and then looked for them, so the list was always empty and the
		// diagnostic always said the device matched.
		const unavailable = [];
		...
		// The effective list is per adapter and derived fresh from the immutable baseline, so
		// running discovery again (the retry-startup path) produces the same result as running
		// it once, and a second adapter instance in the same process is unaffected.
		//
		// Discovery extends the list; it does not shrink it. Whether a section missing from
		// SECTIONS means the device cannot serve it is not established — and the cost of the
		// two readings is wildly asymmetric. Dropping a section the device does serve loses
		// that data silently, and because localInitPollSettings derives the high-priority
		// form from this same list, a response that omitted ENERGY would have stopped live
		// polling entirely. What the device did not advertise is reported in
		// info.unavailableSections rather than acted on.
		//
		// Polling the full list is also the field-proven behaviour: every device has been
		// sent the complete hardcoded list since the poll-logic rewrite in 2023, and the
		// narrowing above arrived with section discovery only in 2026. Union keeps discovery's
		// benefit — picking up sections the shipped list does not know — without the risk.
		adapter.knownSections = new Set([...allKnownObjects, ...newSections]);
		...
		await adapter.doState(
			...
			"Sections this adapter knows that the device did not advertise",
			"",
			false,
			false,
		);
	} catch (error) {
		// A device that restricts or garbles the discovery endpoint must not cost us the
		// sections we already know how to poll — leaving knownSections unset falls back to
		// the full baseline.
