...
async function localSendControl(adapter, stateId, payload, description) {
	...
	try {
		...
		adapter.logError(error, `Failed to control: ${description}`);
	}
}
...
async function localHandleSocketControl(adapter, stateId, socketIdx, field, value) {
	if (adapter.socketCount === undefined || socketIdx >= adapter.socketCount) {
		adapter.log.warn(`Socket ${socketIdx} does not exist (device has ${adapter.socketCount ?? 0} sockets)`);
		return;
	}

	// Non-Apply fields: just store the pending value (no ack)
	if (field !== "Apply") {
		adapter.log.debug(`Socket ${socketIdx}: pending ${field} = ${value}`);
		return;
	}
	...
	adapter.log.info(`Applying socket ${socketIdx} changes via local...`);
	...
	adapter.log.debug(`Socket control payload: ${payload}`);
	...
	adapter.log.info(`Socket ${socketIdx} changes applied via local`);
}
...
async function localCreateSocketControls(adapter) {
	...
	adapter.log.info(`Created control datapoints for ${adapter.socketCount} socket(s)`);
}
...
async function cleanupControlChannels(adapter, pattern, label) {
	...
	for (const ch of channels) {
		if (ch._id && ch._id.includes(pattern)) {
			...
			adapter.log.debug(`Cleaned up ${label} control channel: ${ch._id}`);
		}
	}
}
...
async function localDiscoverAndSyncControls(adapter, obj) {
	...
	if (obj.SOCKETS) {
		if (adapter.socketCount === undefined && typeof obj.SOCKETS.NUMBER_OF_SOCKETS === "number") {
			...
			adapter.log.debug(`Detected ${count} socket(s)`);
	...
	if (obj.WIZARD && obj.WALLBOX) {
		if (adapter.wallboxCount === undefined && typeof obj.WIZARD.SETUP_NUMBER_WALLBOXES === "number") {
			...
			adapter.log.debug(`Detected ${count} wallbox(es)`);
...
async function localHandleWallboxControl(adapter, stateId, wbIdx, field, value) {
	if (adapter.wallboxCount === undefined || wbIdx >= adapter.wallboxCount) {
		adapter.log.warn(`Wallbox ${wbIdx} does not exist (device has ${adapter.wallboxCount ?? 0} wallboxes)`);
		return;
	}

	// Non-Apply fields: just store the pending value (no ack)
	if (field !== "Apply") {
		adapter.log.debug(`Wallbox ${wbIdx}: pending ${field} = ${value}`);
		return;
	}
	...
	adapter.log.info(`Applying wallbox ${wbIdx} changes...`);
	...
	for (const [fieldName, mapping] of Object.entries(fieldMap)) {
		...
		if (mapping.bool) {
			...
			if (isNaN(numVal) || numVal < 0) {
				adapter.log.warn(`Invalid value for wallbox control ${fieldName}: ${val}`);
				continue;
			}
		...
		adapter.log.info(`Wallbox ${wbIdx} ${fieldName} = ${val}`);
	}

	if (Object.keys(wallboxPayload).length > 0) {
		const payload = JSON.stringify({ WALLBOX: wallboxPayload });
		adapter.log.debug(`Wallbox control payload: ${payload}`);
		await localSendControl(adapter, stateId, payload, `applying wallbox ${wbIdx} changes`);
	} else {
		adapter.log.debug(`Wallbox ${wbIdx}: no pending changes to apply`);
	}

	await adapter.setState(`${pfx}.Apply`, { val: false, ack: true });
	adapter.log.info(`Wallbox ${wbIdx} changes applied`);
}
...
async function localCreateWallboxControls(adapter) {
	...
	adapter.log.info(`Created control datapoints for ${adapter.wallboxCount} wallbox(es)`);
}
...
async function localInitPollSettings(adapter) {
	...
	adapter.log.debug(`(localInitPollSettings) lowPrio: ${adapter.lowPrioForm}`);
	...
	adapter.log.debug(`(localInitPollSettings) highPrio: ${adapter.highPrioForm}`);
}
...
function addUserDps(adapter, value, objectsSet, dpToAdd) {
	...
	dpToAdd
		...
		.split(",")
		.forEach((item) => objectsSet.add(item));
	adapter.log.debug(`(addUserDps) Datapoints config changed for ${value}: ${dpToAdd.toUpperCase().trim()}`);
}
...
async function localCheckConnection(adapter) {
	...
	try {
		adapter.log.info(`connecting to Senec (local): ${url}`);
		await localDoGet(adapter, url, form, adapter.config.pollingTimeout, true);
		adapter.log.info(`connected to Senec (local): ${url}`);
		adapter.lalaConnected = true;
	} catch (error) {
		throw new Error(
			`Error connecting to Senec (IP: ${adapter.connectVia}${adapter.config.senecip}). Exiting! (${
				error
...
async function localDiscoverSections(adapter) {
	...
	try {
		adapter.log.info("Discovering available sections from device...");
		...
		if (newSections.length > 0) {
			adapter.log.info(`Discovered ${newSections.length} new section(s): ${newSections.join(", ")}`);
		}
		...
		if (newSections.length === 0 && unavailable.length === 0) {
			adapter.log.info("Section discovery complete. Device matches existing sections.");
		}
		...
		adapter.log.warn(`Section discovery failed (device may restrict access): ${error.message}`);
		await adapter.doState(
			"info.discoveredSections",
			`error: ${error.message}`,
			"Sections discovered beyond hardcoded list",
...
async function localDoGet(adapter, pUrl, pForm, pollingTimeout, isPost) {
	...
	adapter.log.debug(`Calling: ${pUrl}`);
	...
	try {
		...
		adapter.log.silly(`(Poll) received data (${response.status}): ${JSON.stringify(content)}`);

		return JSON.stringify(content);
	} catch (error) {
		if (error.code === "ERR_CANCELED" || error.name === "CanceledError") {
			adapter.log.debug("Request aborted (adapter shutdown)");
...
async function localPoll(adapter, isHighPrio, retry) {
	...
	if (!isHighPrio) {
		adapter.log.info("LowPrio polling (local) ...");
	...
	try {
		...
		if (body.includes('\\"')) {
			...
			adapter.log.debug(`(Poll) Double escapes detected!  Body inc: ${body}`);
			body = body.replace(/\\"/g, '"');
			adapter.log.debug(`(Poll) Double escapes autofixed! Body out: ${body}`);
		}
			...
			if (!adapter.unloaded) {
				adapter.setTimeout(() => {
					localPoll(adapter, isHighPrio, retry).catch((e) =>
						adapter.logError(e, `❌ Local poll failed (highPrio=${isHighPrio})`),
					);
				}, interval);
			}
			return;
		...
		adapter.log.silly(`(Poll) Parsed object: ${JSON.stringify(obj)}`);
		...
		if (retry == adapter.config.retries && adapter.config.retries < 999) {
			adapter.logError(
				error,
				`Error reading from Senec ${isHighPrio ? "high" : "low"}Prio (${adapter.config.senecip}). Retried ${
					retry
			...
			retry += 1;
			const delay = interval * adapter.config.retrymultiplier * retry;
			adapter.log.warn(
				`Error reading from Senec ${isHighPrio ? "high" : "low"}Prio (${adapter.config.senecip}). Retry ${
					retry
				}/${adapter.config.retries} in ${delay / 1000} seconds! (${error})`,
			);
			if (!adapter.unloaded) {
				adapter.setTimeout(() => {
					localPoll(adapter, isHighPrio, retry).catch((e) =>
