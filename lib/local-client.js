...
async function localHandleSocketControl(adapter, stateId, socketIdx, field, value) {
	if (adapter.socketCount === undefined || socketIdx >= adapter.socketCount) {
		adapter.log.warn(`[Local] Socket ${socketIdx} does not exist (device has ${adapter.socketCount ?? 0} sockets)`);
		return;
	}
...
async function localHandleWallboxControl(adapter, stateId, wbIdx, field, value) {
	if (adapter.wallboxCount === undefined || wbIdx >= adapter.wallboxCount) {
		adapter.log.warn(`[Local] Wallbox ${wbIdx} does not exist (device has ${adapter.wallboxCount ?? 0} wallboxes)`);
		return;
	}
	...
	for (const [fieldName, mapping] of Object.entries(fieldMap)) {
		...
		if (mapping.bool) {
			...
			if (isNaN(numVal) || numVal < 0) {
				adapter.log.warn(`[Local] Invalid value for wallbox control ${fieldName}: ${val}`);
				continue;
			}
...
async function localDiscoverSections(adapter) {
	...
	try {
		...
		adapter.log.warn(`[Local] Section discovery failed (device may restrict access): ${error.message}`);
		await adapter.doState(
			"info.discoveredSections",
			`error: ${error.message}`,
			"Sections discovered beyond hardcoded list",
...
async function localPoll(adapter, isHighPrio, retry) {
	...
	if (!isHighPrio) {
		adapter.log.info("[Local] LowPrio polling...");
	...
	try {
		...
		if (retry == adapter.config.retries && adapter.config.retries < 999) {
			adapter.logError(
				error,
				`[Local] ❌ Error reading from Senec ${isHighPrio ? "high" : "low"}Prio (${adapter.config.senecip}). Retried ${
					retry
			...
			adapter.log.warn(
				`[Local] Error reading from Senec ${isHighPrio ? "high" : "low"}Prio (${adapter.config.senecip}). Retry ${
					retry
