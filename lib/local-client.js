...
async function localPoll(adapter, isHighPrio, retry) {
	...
	try {
		...
		retry += 1;
		// Cap retry counter at configured max so backoff plateaus but polling never stops
		const cappedRetry = Math.min(retry, adapter.config.retries || 10);
		const delay = Math.max(10000, interval * adapter.config.retrymultiplier * cappedRetry);

		if (retry === 1 && adapter.lalaConnected) {
