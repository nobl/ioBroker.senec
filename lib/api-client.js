	...
	return adapter.apiQueue.add(async () => {
		...
		for (let attempt = 0; attempt < maxAttempts; attempt++) {
			try {
				...
				// Only a 401 is retried here; everything else is retried by AdaptiveRequestQueue,
				// so reporting an attempt count would misrepresent where the retries happen.
				adapter.log.debug(
