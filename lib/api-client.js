"use strict";
// Endpoints (SENEC App API):
//   Systems:      https://senec-app-systems-proxy.prod.senec.dev/systems/api/v1
//   Measurements: https://senec-app-measurements-proxy.prod.senec.dev/measurements/api/v1
//   Abilities:    https://senec-app-abilities-proxy.prod.senec.dev/abilities/api/v1
//   Wallbox:      https://senec-app-wallbox-proxy.prod.senec.dev/wallbox/api/v1
...
/** @typedef {import('axios').AxiosResponse} AxiosResponse */ // eslint-disable-line jsdoc/check-tag-names
...
/**
 * Extra attempts for a GET that failed transiently. Kept small: the queue already backs off
 * on 429 and timeouts, and a poll cycle that cannot get through recovers on its next run.
 */
const API_GET_RETRIES = 2;

/**
 * What the login flow asks the SSO to answer with.
 *
 * Every step of it is a browser flow — the authorization request and both form posts are answered
 * with an HTML page — while the shared client defaults to `application/json`, which suits the
 * portal's JSON calls and the token endpoint but is not what a login form is served under.
 * Keycloak negotiates content, most visibly on the paths that report an error, so the login asks
 * for what it actually parses. Set per request rather than on the client: the same instance
 * carries the mein-senec.de connector, whose calls are JSON and must keep their own expectation.
 */
const BROWSER_ACCEPT = "text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8";

/** The token endpoint answers JSON, and says so explicitly rather than relying on the default. */
const TOKEN_HEADERS = {
...
async function apiStartTokenManager(adapter) {
	try {
		...
		// Only the stored-token read is guarded here. Wrapping the refresh in the same try
		// used to make its failure trigger a second full login, on top of the one
		// apiRefreshToken had already made.
		adapter.log.warn(`[API] ⚠️ Stored refresh token could not be read: ${error.message}`);
		adapter.refreshToken = null;
	}

	// No refresh token at all → full login. Nothing else is retrying in this case, so the
	// result is reported as-is and the connector retry picks it up.
	if (!adapter.refreshToken) {
		adapter.log.info("[API] 🔐 No refresh token present. Performing full login...");
		return !!(await apiLogin(adapter));
	}

	adapter.log.info("[API] 🔐 Using existing refresh token. Trying initial token refresh...");
	try {
		await apiRefreshToken(adapter);
	} catch (error) {
		// apiRefreshToken owns recovery from a failed refresh: it has already made the single
		// fallback login that invalid_grant warrants, and armed a backed-off retry. Logging in
		// again from here would repeat that request against an SSO that just refused it.
		adapter.log.warn(`[API] ⚠️ Initial token refresh failed: ${error.message}`);
	}
	...
	// Startup recovery belongs to the connector retry, because it re-runs the whole init and
	// restarts polling — restoring a token on its own would leave the adapter authenticated
	// but idle. Stand the token-level retry down so the two do not run as parallel loops,
	// each re-attempting the login the other just made.
...
/**
 * The account values that must never reach a log, in the shape the auth helpers expect.
 *
 * Collected in one place so a new log site cannot forget one of them: a call that passes only the
 * address leaves an echoed password unmasked, and the redaction would still look like it was done.
 *
 * @param {SenecAdapter} adapter - Senec adapter instance
 * @returns {{mail?: string, password?: string}} the configured account values
 */
function authSecrets(adapter) {
	return { mail: adapter.config.api_mail, password: adapter.config.api_pwd };
}

/**
 * Put a login page into the debug log, redacted.
 *
 * The one-line reason `describeAuthFailure` produces says *that* a step failed, not what the SSO
 * put in front of us — and when the answer is "an identity provider we have never seen", the
 * fields of the form and the names on the error page are the only place that shows up.
 *
 * A page from a *failed* step is written whenever the log level allows it (`always`), because it
 * cannot be asked for after the event: by the time a report arrives, the login that produced it
 * is long gone, and requiring a settings change first costs a round trip per report. The pages of
 * a login that is working are verbose and say nothing, so those stay behind the request/response
 * option. Repeats are collapsed — a login failing the same way on every retry would otherwise
 * write the same page every few seconds, and the retry back-off has full jitter, so "every few
 * seconds" is not hyperbole.
 *
 * @param {SenecAdapter} adapter - Senec adapter instance
 * @param {string} label - what this page is, e.g. "login form"
 * @param {unknown} html - the response body
 * @param {{always?: boolean}} [options] - `always` ignores the request/response option
 * @returns {void}
 */
function logAuthPage(adapter, label, html, options = {}) {
	if (!options.always && !adapter.config.api_reqnresp_log) {
		return;
	}
	if (typeof html !== "string" || !html) {
		// The token exchange fails with a JSON body, so there is no page to write. Dropping the key
		// as well is what stops a later, unrelated HTML failure from being collapsed against the
		// page of a failure that is already over.
		if (options.always) {
			adapter.lastLoggedAuthPage = null;
		}
		return;
	}
	// Everything below is a full-body regex sweep and a hash over the result, and at any level
	// above debug the outcome is discarded by the logger. This guards both routes, not just the
	// `always` one: a page logged under the request/response option is just as wasted when the
	// level throws it away. `undefined` means the adapter double in front of us does not report a
	// level — proceed, rather than disable the diagnostic silently.
	const level = adapter.log?.level;
	if (level !== undefined && level !== "debug" && level !== "silly") {
		return;
	}
	try {
		const page = redactAuthHtml(html, authSecrets(adapter));
		// The label is part of the key because two different steps can fail on the same body, and
		// collapsing those reports the second one as "unchanged" — the opposite of what it means.
		// Its query string is dropped for the same reason the page's is: a label naming a step
		// carries `tab_id`, which `redactAuthUrl` keeps on purpose to stay readable but which is new
		// on every attempt — hashed verbatim it defeats the collapse on the one failure that repeats
		// most, the credential post the SSO refuses on every retry.
		// sha256 rather than sha1: sha1 throws on a FIPS-enabled Node, and the catch below would
		// then turn a hardened installation into one with no login diagnostics at all.
		const key = crypto
			.createHash("sha256")
			.update(`${label.replace(/\?\S*/g, "")}\0${authPageFingerprint(page)}`)
			.digest("hex");
		if (options.always && key === adapter.lastLoggedAuthPage) {
			adapter.log.debug(`[SSO PAGE] ${label}: unchanged since the last attempt.`);
			return;
		}
		if (options.always) {
			adapter.lastLoggedAuthPage = key;
		}
		adapter.log.debug(`[SSO PAGE] ${label}: ${page}`);
	} catch (err) {
		adapter.log.debug(`SSO page logging failed: ${err.message}`);
	}
}

/**
 * Report a login step that failed on a response the SSO considered a success.
 *
 * Every structural failure of the flow — a form that is not there, a field that is missing, a
 * redirect that goes somewhere else — arrives as a 2xx or a 3xx, so `authStep` never sees it and
 * the page that would explain it is dropped. Dumping it here gives those failures the same
 * diagnostics as an HTTP error, from one place, so no step can be added later that quietly has
 * none.
 *
 * Returns the error instead of throwing it, so that the guard at the call site still narrows the
 * value it just checked.
 *
 * @param {SenecAdapter} adapter - Senec adapter instance
 * @param {string} label - what the page is, e.g. "page after the username step"
 * @param {unknown} html - the body the step had in hand
 * @param {string} message - the error message
 * @returns {Error} the error for the caller to throw
 */
function failLogin(adapter, label, html, message) {
	logAuthPage(adapter, label, html, { always: true });
	return new Error(message);
}

/**
 * Say so when a login form sends the credentials somewhere other than the SENEC SSO.
 *
 * The form action decides which host receives the account password, and the adapter follows
 * whatever the page names. A brokered identity provider is a legitimate and expected shape here,
 * so this does not refuse the post — but the destination is exactly what an account that logs in
 * through a third party has to be diagnosed from, and it must not be something only a packet
 * capture reveals.
 *
 * @param {SenecAdapter} adapter - Senec adapter instance
 * @param {string} url - the form action, absolute or relative to the SSO
 * @returns {void}
 */
function warnOnForeignFormAction(adapter, url) {
	let target;
	let sso;
	try {
		// Resolved against the SSO, because a relative action stays on the SSO's own origin and
		// comparing it as a bare string would report every one of those as foreign.
		target = new URL(url, CONFIG.authUrl);
		sso = new URL(CONFIG.authUrl);
	} catch {
		// An action neither absolute nor resolvable is not a host that can be named; the post that
		// follows fails on its own and is reported by authStep.
		return;
	}
	if (target.origin !== sso.origin) {
		adapter.log.warn(
			`[API] ⚠️ The login form posts to ${target.host}, not to ${sso.host} — ` +
				`the account credentials go to ${redactAuthUrl(target.toString())}.`,
		);
	}
}

/**
 * Run one request of the login flow and, if it fails, say which request it was and what the SSO
 * answered.
 *
 * Without this every step of the flow fails with the same "Request failed with status code 400",
 * which names neither the endpoint nor the reason — so a report of a failing login cannot be
 * acted on at all. The original error is kept as `cause` so the retry logic can still read
 * `err.response.status`.
 *
 * @param {SenecAdapter} adapter - Senec adapter instance
 * @param {string} step - description of the request, already redacted
 * @param {() => Promise<AxiosResponse>} run - the request
 * @returns {Promise<AxiosResponse>} the response
 */
async function authStep(adapter, step, run) {
	try {
		return await run();
	} catch (e) {
		logAuthPage(adapter, `body of the failed step (${step})`, e?.response?.data, { always: true });
		const err = new Error(`${step} → ${describeAuthFailure(e, authSecrets(adapter))}`, { cause: e });
		// Carried over so callers that branch on the HTTP status keep working through the wrap.
		// @ts-expect-error — augmenting a plain Error with the axios response
...
async function apiLogin(adapter) {
	...
	// Masked, and written before the first request: a login that is refused says which account it
	// was refused for, which is the one thing a report of "my credentials are correct" cannot
	// otherwise establish.
	adapter.log.debug(`[API] 🔐 Logging in as ${maskMailAddress(adapter.config.api_mail)}`);
	...
	try {
		...
		const pageRes = await authStep(adapter, `GET ${CONFIG.authUrl} (authorization request)`, () =>
			authClient.get(`${CONFIG.authUrl}?${authParams}`, {
				jar: adapter.jar,
				headers: { Accept: BROWSER_ACCEPT },
			}),
		);
		logAuthPage(adapter, "login form as served", pageRes.data);
		// One match per page, and the action, the predicates and the hidden fields all read from it.
		// Scanning the whole document for the fields while taking the action from a single form is
		// what let a page with two forms send the credentials to one form's action with the other
		// form's body.
		const loginForm = matchLoginForm(pageRes.data);
		if (!loginForm) {
			throw failLogin(adapter, "login form as served", pageRes.data, "Login-Form URL not found.");
		}

		const postForm = (step, url, data) => {
			warnOnForeignFormAction(adapter, url);
			// Names only, never values — but the names are what decides whether the adapter posts
			// the form the SSO served. Reconstructing that from outside took a standalone script
			// and a round trip through the reporter; one line does it from the log.
			adapter.log.debug(`[SSO POST] ${step} sends ${JSON.stringify([...data.keys()])}`);
			return authStep(adapter, `POST ${redactAuthUrl(url)} (${step})`, () =>
		...
		if (!hasUsername(loginForm.inner)) {
			throw failLogin(
				adapter,
				"login form as served",
				pageRes.data,
				"Expected: Login-Form with username. Got something else.",
			);
		}

		// Step 1 (username only or full form)
		const singleStep = Boolean(hasUsernameAndPassword(loginForm.inner));
		const formData = buildLoginForm(loginForm.inner, {
			username: adapter.config.api_mail,
			password: singleStep ? adapter.config.api_pwd : undefined,
		});
		loginRes = await postForm(singleStep ? "credentials" : "username", loginForm.action, formData);

		// Step 2 (password step if required)
		if (!singleStep) {
			logAuthPage(adapter, "page after the username step", loginRes.data);

			// A 3xx here has no body, so the page dump has nothing to write and the Location header
			// is the only thing naming where the SSO went — which for a brokered account is the
			// identity provider itself, the one value the failure has to be diagnosed from.
			const location = loginRes.headers?.location;
			if (location) {
				adapter.log.debug(`[SSO PAGE] the username step was answered with ${redactAuthUrl(location)}`);
			}
			// Only appended when there is one, so the plain "no password form" failure keeps the
			// wording it has always had.
			const redirectHint = location ? ` SSO answered with ${redactAuthUrl(location)}.` : "";

			const passwordForm = matchLoginForm(loginRes.data);
			if (!passwordForm || !hasPassword(passwordForm.inner)) {
				throw failLogin(
					adapter,
					"page after the username step",
					loginRes.data,
					`Expected: Login-Form with password. Got something else.${redirectHint}`,
				);
			}
			...
			loginRes = await postForm(
				"password",
				passwordForm.action,
				buildLoginForm(passwordForm.inner, {
					username: adapter.config.api_mail,
		...
		// The page is identified from the whole document — an OTP input anywhere means the SSO is
		// asking for a code, and saying so is what tells a 2FA user to configure a secret. The code
		// is then posted to the form that carries it, action and hidden fields from the same match.
		if (
			!loginRes.headers.location &&
			loginRes.status === 200 &&
			typeof loginRes.data === "string" &&
			hasOtp(loginRes.data)
		) {
			...
			const otpForm = matchLoginForm(loginRes.data);
			if (!otpForm) {
				throw failLogin(
					adapter,
					"page with the TOTP form",
					loginRes.data,
					"TOTP form found but could not extract form action URL.",
				);
			}
			const otpCode = generateTOTP(adapter.config.api_totp_secret);
			loginRes = await postForm("TOTP", otpForm.action, buildLoginForm(otpForm.inner, { otp: otpCode }));
		}
		...
		// A redirect that does not go to the app's own scheme means the SSO is not done with us:
		// another authentication step, a required action, or an error page. Naming where it sent
		// us is the difference between a report that can be diagnosed and one that cannot.
		if (!redirectLocation.startsWith("senec-app-auth://")) {
			throw failLogin(
				adapter,
				"page carrying the unexpected redirect",
				loginRes.data,
				`Login incomplete: SSO redirected to ${redactAuthUrl(redirectLocation)} instead of the app redirect URI. ` +
					"This usually means the account needs a further login step the adapter does not handle yet.",
			);
		}

		const authCode = new URL(redirectLocation.replace("senec-app-auth://", "https://")).searchParams.get("code");
		if (!authCode) {
			throw new Error(`Authorization code not found in redirect (${redactAuthUrl(redirectLocation)}).`);
		}

		const tokenRes = await authStep(adapter, `POST ${CONFIG.tokenUrl} (authorization_code exchange)`, () =>
			authClient.post(
				CONFIG.tokenUrl,
				new URLSearchParams({
		...
		// A login that got through ends the run of identical failures, so the next one that fails
		// is written out in full rather than collapsed against a page from before the recovery.
		adapter.lastLoggedAuthPage = null;
...
function scheduleTokenRefresh(adapter) {
	...
	if (!adapter.unloaded) {
		adapter.log.debug(
			`[API] 🔐 Next token refresh in ${(delay / 1000).toFixed(0)}s ` +
		...
		adapter.timerTokenRefresh =
			adapter.setTimeout(() => {
				apiRefreshToken(adapter).catch((err) => {
					adapter.log.debug(`[API] ⚠ Token refresh failed: ${err.message}`);
				});
			}, delay) ?? null;
	}
}
...
async function apiRefreshToken(adapter) {
	...
	if (adapter.refreshPromise) {
		adapter.log.debug("[API] 🔐 Refresh already in progress, waiting for it to complete...");
	...
	adapter.refreshPromise = (async () => {
		if (!refreshToken) {
			// Nothing to refresh — a full login is the only way back. Handled inside the
			// single flight so that several callers arriving together cause one login
			// rather than one login each.
			adapter.log.debug("[API] 🔐 No refresh token available — full login required.");
			if (await apiLogin(adapter)) {
				return;
			}
			adapter.authBlocked = true;
			scheduleTokenRetry(adapter);
			throw new Error("Login failed and no refresh token is available.");
		}

		try {
			adapter.log.debug("[API] 🔐 Refreshing API token...");
			...
			adapter.tokenFailureCount = 0;
			adapter.authBlocked = false;
			// A refresh is a recovery too, so it ends the run of identical failures the same way a
			// login does — otherwise the next failing login is collapsed against a page from before.
			adapter.lastLoggedAuthPage = null;
			...
			if (adapter.config.api_showPolling) {
				...
				adapter.log.debug(`[API] ✅ Token refreshed. Expires in ${expiresIn}s`);
			}
			...
			// A refresh token the SSO no longer accepts is routine — it happens after every
			// server-side session expiry — and the full login that follows is the normal cure,
			// not a symptom. Logging it at the same level as a genuine failure is what makes an
			// ordinary re-login look like an outage in the log.
			if (errorCode === "invalid_grant") {
				adapter.log.debug(
					`[API] 🔐 Stored refresh token no longer accepted: ${describeAuthFailure(err, authSecrets(adapter))}`,
				);
			} else {
				adapter.log.warn(`[API] ⚠️ Token refresh failed: ${describeAuthFailure(err, authSecrets(adapter))}`);
			}

			if (errorCode === "invalid_grant" || status === 400) {
				adapter.log.info("[API] 🔐 Refresh token no longer valid → performing full login.");
				if (await apiLogin(adapter)) {
					return;
				}
				// A login that returned nothing is not a recovery. Falling through to the
				// shared retry path keeps the auth block in place and leaves a scheduled
				// attempt behind, so the adapter cannot strand itself with no token, no
				// timer and no error to show for it.
				adapter.log.warn("[API] ⚠️ Full login after an invalid refresh token failed.");
			}
...
/**
 * Schedule another authentication attempt after a failure, with growing backoff.
 *
 * Every failing path goes through here, so no failure can end without a future attempt —
 * and the growing delay is what stops bad credentials or a missing TOTP secret from turning
 * into a login loop against the SSO.
 *
 * @param {SenecAdapter} adapter - Senec adapter instance
 * @returns {void}
 */
function scheduleTokenRetry(adapter) {
...
async function apiPoll(adapter) {
	...
	if (!adapter.config.api_use || adapter.unloaded) {
		adapter.log.info("[API] Usage of SENEC App API not configured.");
		return;
	}
	...
	try {
		if (adapter.authBlocked) {
			adapter.log.debug("[API] ⏸ Poll skipped - authentication currently recovering.");
		...
		if (cycleResult.partialFailure) {
			adapter.log.warn(
				`[API] ⚠️ Partial failure: ${cycleResult.failedSystems} system(s) failed, ` +
					`at least one scheduled task succeeded.`,
			);
		}

		adapter.apiFailureCount = 0;
		if (!adapter.apiConnected) {
			adapter.apiConnected = true;
		...
		if (adapter.config.api_debug_states) {
			try {
				...
				adapter.log.debug(`[API] Failed to update queue stats: ${statsError.message}`);
			}
		}
		...
		if (adapter.config.api_debug_log) {
			try {
				...
				adapter.log.debug(`[API] Failed to log queue stats: ${logError.message}`);
			}
		}
...
function apiScheduleNextPoll(adapter, delay) {
	...
	adapter.log.debug(`[API] ⏱ Next API poll scheduled in ${(delay / 1000).toFixed(0)}s`);
}
...
function apiBuildPollContext(adapter) {
	...
	// Day windows are local. doMeasurementsDay derives its end-of-day from this date with
	// local getters and sends the pair as an absolute ISO range, so the day named here has
	// to be the plant's own calendar day. Reading the UTC fields and handing them to the
	// local constructor named the previous day for the part of the night in which UTC has
	// not rolled over yet — in Europe/Berlin, every day between midnight and 02:00.
	// Passing date - 1 to the constructor is calendar arithmetic, so it stays correct across
	// month and year boundaries and across DST, where a day is not 24 hours long.
	const today = new Date(now.getFullYear(), now.getMonth(), now.getDate(), 0, 0, 0, 0);
	const yesterday = new Date(now.getFullYear(), now.getMonth(), now.getDate() - 1, 0, 0, 0, 0);

	// Month and year stay on UTC deliberately: doMeasurementsMonth reads the date back with
	// getUTC* getters and doMeasurementsYear builds its range with Date.UTC. Making these
	// local without changing those functions would shift each window by the UTC offset.
...
async function apiEnsureSystemsLoaded(adapter) {
	...
	adapter.log.debug("[API] 🔄 Reading available systems from API ...");
	...
	for (const sys of sysRes.data) {
		adapter.log.debug(`[API] System found: ${JSON.stringify(sys)}`);
...
async function apiRunPollCycle(adapter) {
	...
	if (adapter.config.api_showPolling) {
		...
		adapter.log.debug("[API] 🔄 Polling SENEC App API...");
	}
...
async function apiPollSingleSystem(adapter, anlagenId, ctx, rebuildAlreadyExecuted) {
	...
	adapter.log.debug(`[API] 🔄 Polling system ${anlagenId}...`);

	if (ctx.shouldRunDashboard) {
		adapter.log[logType](`[API] 🔄 Polling system ${anlagenId} - Dashboard`);
	...
	if (ctx.shouldRunDetails) {
		adapter.log[logType](`[API] 🔄 Polling system ${anlagenId} - Details (day values)`);
	...
	if (ctx.shouldRunHeavy) {
		adapter.log[logType](`[API] 🔄 Polling system ${anlagenId} - Heavy (month / year values)`);
	...
	// A system counts as failed when any tier that was scheduled did not fully succeed.
	// Reading it as "every scheduled tier failed" instead would make the single-plant case
	// unreportable — a plant whose dashboard fails while its details succeed is precisely
	// the situation the partial-failure warning exists for. A cycle with nothing scheduled
	// is idle, not failed.
	result.failed =
		(result.dashboardScheduled && !result.dashboardSucceeded) ||
		(result.detailsScheduled && !result.detailsSucceeded) ||
		(result.heavyScheduled && !result.heavySucceeded);
...
async function apiPollDashboard(adapter, anlagenId) {
	try {
		...
		adapter.log.silly(`[API] DashRes keys: ${Object.keys(dashRes.data).join(", ")}`);
...
async function _apiPollEndpoint(adapter, anlagenId, url, evalPrefix, pollName) {
	try {
		...
		adapter.log.debug(`[API] ${pollName} polled for ${anlagenId}`);
	} catch (error) {
...
async function apiPollAbilities(adapter, anlagenId) {
	try {
		...
		adapter.log.debug(`[API] Abilities polled for ${anlagenId}`);
	} catch (error) {
...
async function apiPollWallboxSearch(adapter, anlagenId) {
	try {
		...
		if (!Array.isArray(res.data)) {
			adapter.log.debug(`[API] Wallbox search returned non-array response for ${anlagenId}`);
			return;
		}
		if (res.data.length === 0) {
			adapter.log.debug(`[API] No wallboxes found for ${anlagenId}`);
			return;
		}
...
async function apiHandleWallboxControl(adapter, wbIdx, field, value) {
	if (wbIdx >= adapter.apiWallboxCount || !adapter.apiWallboxSystemId) {
		adapter.log.warn(`[API] ⚠️ Wallbox ${wbIdx} does not exist`);
		return;
	}
	...
	if (!uuid) {
		adapter.log.warn(`[API] ⚠️ No UUID for wallbox ${wbIdx}`);
		return;
	}
		...
		if (targetMode) {
			...
			if (targetMode === "LOCKED" && !currentlyLocked) {
				...
				if (currentlyLocked) {
					...
					if (!unlockRes?.data) {
						adapter.log.warn(`[API] ⚠️ Unlock failed for wallbox ${wbIdx}`);
...
async function apiPollDataAvailability(adapter, anlagenId) {
	try {
		...
		adapter.log.debug(`[API] Data availability polled for ${anlagenId}`);
	} catch (error) {
	...
	try {
		...
		if (adapter.config.api_debug_log) {
			for (const r of results) {
				adapter.log.debug(`[API] ${pollName} ${anlagenId} / ${r.label}: ${r.status}`);
			}
		}
		...
		adapter.log.debug(
			`[API] ${pollName} summary ${anlagenId}: ${m.formatMeasurementSummary(summary)} | ${m.formatMeasurementClassification(classification)}`,
		);
...
function apiFinalizePollTimestamps(adapter, result) {
	...
	// Leaving the timestamp untouched is what makes the tier due again on the next cycle.
	// Without a word about it the log shows the failure and nothing else, which reads as if
	// the data were simply lost until the interval comes round again.
	const finalize = (type, scheduled, succeeded) => {
		if (!scheduled) {
			return;
		}
		if (succeeded === systemsCount) {
			apiMarkPollTimestamp(adapter, type);
			return;
		}
		adapter.log.info(
			`[API] ↩ ${type} incomplete (${succeeded}/${systemsCount} systems) — retried on the next poll cycle.`,
		);
	};

	finalize("dashboard", result.dashboardScheduled, result.dashboardSucceeded);
	finalize("details", result.detailsScheduled, result.detailsSucceeded);
	finalize("heavy", result.heavyScheduled, result.heavySucceeded);
}

/**
 * Whether an API error is worth another attempt.
 *
 * Only failures that say something about the moment rather than about the request itself:
 * timeouts, rate limiting, server-side errors and dropped connections. A 400 or a 404 means
 * the request cannot succeed no matter how often it is sent, and a 401 never reaches here
 * because _apiRequest refreshes the token and retries in place.
 *
 * @param {Error & { name?: string; code?: string; response?: { status?: number } }} err - The error thrown by the request
...
function isTransientApiError(err) {
	...
	// No response at all — a transport failure. Abort is the adapter shutting down.
	if (err?.name === "AbortError" || err?.name === "CanceledError") {
		return false;
	}
	const transportCodes = ["ECONNABORTED", "ETIMEDOUT", "ECONNRESET", "EAI_AGAIN", "ENETUNREACH", "EPIPE"];
	return (!!err?.code && transportCodes.includes(err.code)) || /timeout/i.test(err?.message || "");
}
 ...
 * - GET is retried by AdaptiveRequestQueue on transient errors. POST and PATCH are not:
 *   they are control commands, and replaying one could act on the appliance twice.
 *
 * @param {SenecAdapter} adapter - Senec adapter instance
...
async function _apiRequest(adapter, method, url, data, config = {}) {
	...
	// Reading can be repeated safely; writing cannot.
	const queueOptions = {
		maxRetries: method === "get" ? API_GET_RETRIES : 0,
		label: `${label} ${url}`,
	...
	return adapter.apiQueue.add(async () => {
		...
		if (adapter.tokenExpiresAt && Date.now() >= adapter.tokenExpiresAt - adapter.baseTime) {
			adapter.log.debug("[API] 🔐 Token close to expiry. Refreshing before request...");
			await apiRefreshToken(adapter);
		}

		if (!adapter.currentToken) {
			adapter.log.debug("[API] 🔐 No current token. Refreshing before request...");
			await apiRefreshToken(adapter);
		}
		...
		for (let attempt = 0; attempt < maxAttempts; attempt++) {
			try {
				...
				if (status === 401 && attempt < maxAttempts - 1) {
					adapter.log.debug("[API] 🔐 401 received. Refreshing token...");
					await apiRefreshToken(adapter);
					continue;
				}
				...
				// Only a 401 is retried here. A transient failure on a GET is retried by
				// AdaptiveRequestQueue, which logs its own attempt count — so reporting this
				// loop's attempt number would misrepresent where the retries happen.
				adapter.log.debug(
					`[API] ${label} request failed${attempt > 0 ? ` after token refresh` : ""} ` +
