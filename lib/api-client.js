...
async function apiStartTokenManager(adapter) {
	try {
		...
		if (!adapter.refreshToken) {
			adapter.log.info("🔐 No refresh token present. Performing full login...");
			const token = await apiLogin(adapter);
			return !!token;
		}
		adapter.log.info("🔐 Using existing refresh token.");

		// We have a refresh token → try refresh
		adapter.log.info("🔐 Trying initial token refresh...");
		await apiRefreshToken(adapter);
		return !!adapter.currentToken;
	} catch (error) {
		adapter.log.warn(`⚠️ Initial refresh failed. Falling back to full login... ${error.message}`);
		const token = await apiLogin(adapter);
		return !!token;
	}
}
...
async function apiLogin(adapter) {
	adapter.log.info("🔄 Start Senec API Login Flow...");
	...
	try {
		...
		if (!loginRes.headers.location && loginRes.status === 200 && loginRes.data && hasOtp(loginRes.data)) {
			...
			adapter.log.info("🔐 2FA/TOTP required. Submitting TOTP code...");
		...
		adapter.logError(e, "❌ Login Error");
		return null;
	}
}
	...
	adapter.refreshPromise = (async () => {
		try {
			...
			if (adapter.config.api_showPolling) {
				adapter.log.info(`✅ Token refreshed. Expires in ${expiresIn}s`);
			} else {
			...
			adapter.log.warn(`⚠️ Token refresh failed: ${err.message} (HTTP ${status || "unknown"})`);

			if (errorCode === "invalid_grant" || status === 400) {
				adapter.log.warn("⚠️ Refresh token invalid → full login required.");
				await apiLogin(adapter);
				return;
			}
...
async function apiPoll(adapter) {
	...
	if (!adapter.config.api_use || !adapter.apiConnected || adapter.unloaded) {
		adapter.log.info("Usage of SENEC App API not configured or not connected.");
		return;
	}
	...
	try {
		...
		adapter.logError(err, `🚨 API Poll failed - ⚠️ Failure count: ${adapter.apiFailureCount}`);
		...
		adapter.log.warn(`⏱ Backoff delay: Retry ${adapter.apiFailureCount} in ${(nextDelay / 1000).toFixed(0)}s`);
	} finally {
...
function apiScheduleNextPoll(adapter, delay) {
	...
	adapter.timerAPI =
		adapter.setTimeout(() => {
			apiPoll(adapter).catch((e) => adapter.logError(e, "❌ Scheduled API poll failed"));
		}, delay) ?? null;
...
async function apiRunPollCycle(adapter) {
	...
	if (adapter.config.api_showPolling) {
		adapter.log.info("🔄 Polling SENEC App API...");
	} else {
...
async function apiPollSingleSystem(adapter, anlagenId, ctx, rebuildAlreadyExecuted) {
	...
	if (adapter.isRebuildEnabled() && !adapter.rebuildRunning && !rebuildAlreadyExecuted) {
		try {
			await adapter.doRebuild(anlagenId);
			result.rebuildExecuted = true;
		} catch (rebuildError) {
			adapter.logError(rebuildError, `❌ Rebuild for system ${anlagenId} failed.`);
		}
	}
...
async function apiPollDashboard(adapter, anlagenId) {
	try {
		...
		adapter.logError(error, `❌ Dashboard poll failed for ${anlagenId}`);
		throw error;
	}
}
...
async function _apiPollEndpoint(adapter, anlagenId, url, evalPrefix, pollName) {
	try {
		...
		adapter.logError(error, `❌ ${pollName} poll failed for ${anlagenId}`);
		throw error;
	}
}
...
async function apiPollAbilities(adapter, anlagenId) {
	try {
		...
		adapter.logError(error, `❌ Abilities poll failed for ${anlagenId}`);
	}
}
...
async function apiPollWallboxSearch(adapter, anlagenId) {
	try {
		...
		adapter.log.info(`Found ${adapter.apiWallboxCount} wallbox(es) via API for ${anlagenId}`);
		...
		adapter.logError(error, `❌ Wallbox search failed for ${anlagenId}`);
...
async function apiCreateWallboxControls(adapter) {
	...
	adapter.log.info(`Created API wallbox control datapoints for ${adapter.apiWallboxCount} wallbox(es)`);
}
...
async function apiHandleWallboxControl(adapter, wbIdx, field, value) {
	...
	if (!uuid) {
		adapter.log.warn(`No UUID for API wallbox ${wbIdx}`);
		return;
	}
	...
	try {
		...
		if (targetMode) {
			...
			if (targetMode === "LOCKED" && !currentlyLocked) {
				adapter.log.info(`Locking API wallbox ${wbIdx}...`);
				...
				if (currentlyLocked) {
					adapter.log.info(`Unlocking API wallbox ${wbIdx} before mode change...`);
					const unlockRes = await apiPatch(adapter, `${baseUrl}/locked/false`);
					if (!unlockRes?.data) {
						adapter.log.warn(`Unlock failed for API wallbox ${wbIdx}`);
				...
				if (targetMode !== currentMode) {
					adapter.log.info(`Setting API wallbox ${wbIdx} mode to ${targetMode}...`);
		...
		if (activeMode === "SOLAR") {
			...
			adapter.log.info(`Applying API wallbox ${wbIdx} solar settings...`);
			...
			if (pendingAllowIntercharge != null) {
				adapter.log.info(`Applying API wallbox ${wbIdx} fast settings...`);
			...
			adapter.log.info(`Applying API wallbox ${wbIdx} comfort settings...`);
		...
		adapter.logError(error, `Failed to apply API wallbox ${wbIdx} changes`);
		await adapter.setState(`${pfx}.Apply`, { val: false, ack: true });
	}
}
...
async function apiPollDataAvailability(adapter, anlagenId) {
	try {
		...
		adapter.logError(error, `❌ Data availability poll failed for ${anlagenId}`);
		throw error;
	}
}
...
async function _runMeasurementTasks(adapter, anlagenId, tasks, pollName, beforeLastPoll) {
	try {
		...
		const summary = adapter.summarizeMeasurementResults(results);
		const classification = adapter.classifyMeasurementSummary(summary);

		adapter.log.debug(
			`${pollName} summary ${anlagenId}: ${adapter.formatMeasurementSummary(summary)} | ${adapter.formatMeasurementClassification(classification)}`,
		);
...
async function apiPollDetails(adapter, anlagenId, ctx) {
	const tasks = [
		{ fn: () => adapter.doMeasurementsDay(anlagenId, ctx.today, "today"), label: "today" },
		{ fn: () => adapter.doMeasurementsDay(anlagenId, ctx.today, "today.hourly"), label: "today.hourly" },
		{ fn: () => adapter.doMeasurementsDay(anlagenId, ctx.yesterday, "yesterday"), label: "yesterday" },
		{
			fn: () => adapter.doMeasurementsDay(anlagenId, ctx.yesterday, "yesterday.hourly"),
	...
	for (let i = 0; i < adapter.apiWallboxUuids.length; i++) {
		...
		tasks.push({
			fn: () => adapter.doMeasurementsDay(anlagenId, ctx.today, "today", wb),
			label: `wb${i}.today`,
		});
		tasks.push({
			fn: () => adapter.doMeasurementsDay(anlagenId, ctx.today, "today.hourly", wb),
			label: `wb${i}.today.hourly`,
		});
		tasks.push({
			fn: () => adapter.doMeasurementsDay(anlagenId, ctx.yesterday, "yesterday", wb),
			label: `wb${i}.yesterday`,
		});
		tasks.push({
			fn: () => adapter.doMeasurementsDay(anlagenId, ctx.yesterday, "yesterday.hourly", wb),
...
async function apiPollHeavy(adapter, anlagenId, ctx) {
	const tasks = [
		{
			fn: () => adapter.doMeasurementsMonth(anlagenId, ctx.currentMonth, "current_month"),
			label: "current_month",
		},
		{
			fn: () => adapter.doMeasurementsMonth(anlagenId, ctx.currentMonth, "current_month.daily"),
			label: "current_month.daily",
		},
		{
			fn: () => adapter.doMeasurementsMonth(anlagenId, ctx.lastMonth, "previous_month"),
			label: "previous_month",
		},
		{
			fn: () => adapter.doMeasurementsMonth(anlagenId, ctx.lastMonth, "previous_month.daily"),
			label: "previous_month.daily",
		},

		{ fn: () => adapter.doMeasurementsYear(anlagenId, ctx.utcYear, false), label: "year" },
		{ fn: () => adapter.doMeasurementsYear(anlagenId, ctx.utcYear, true), label: "year.monthly" },
		{ fn: () => adapter.doMeasurementsYear(anlagenId, ctx.utcYear - 1, false), label: "prev_year" },
		{ fn: () => adapter.doMeasurementsYear(anlagenId, ctx.utcYear - 1, true), label: "prev_year.monthly" },
	];
	...
	for (let i = 0; i < adapter.apiWallboxUuids.length; i++) {
		const wb = { uuid: adapter.apiWallboxUuids[i], index: i };
		tasks.push({
			fn: () => adapter.doMeasurementsMonth(anlagenId, ctx.currentMonth, "current_month", wb),
			label: `wb${i}.current_month`,
		});
		tasks.push({
			fn: () => adapter.doMeasurementsMonth(anlagenId, ctx.currentMonth, "current_month.daily", wb),
			label: `wb${i}.current_month.daily`,
		});
		tasks.push({
			fn: () => adapter.doMeasurementsMonth(anlagenId, ctx.lastMonth, "previous_month", wb),
			label: `wb${i}.previous_month`,
		});
		tasks.push({
			fn: () => adapter.doMeasurementsMonth(anlagenId, ctx.lastMonth, "previous_month.daily", wb),
			label: `wb${i}.previous_month.daily`,
		});
		tasks.push({
			fn: () => adapter.doMeasurementsYear(anlagenId, ctx.utcYear, false, wb),
			label: `wb${i}.year`,
		});
		tasks.push({
			fn: () => adapter.doMeasurementsYear(anlagenId, ctx.utcYear, true, wb),
			label: `wb${i}.year.monthly`,
		});
		tasks.push({
			fn: () => adapter.doMeasurementsYear(anlagenId, ctx.utcYear - 1, false, wb),
			label: `wb${i}.prev_year`,
		});
		tasks.push({
			fn: () => adapter.doMeasurementsYear(anlagenId, ctx.utcYear - 1, true, wb),
			label: `wb${i}.prev_year.monthly`,
		});
	}

	await _runMeasurementTasks(adapter, anlagenId, tasks, "Heavy", () => adapter.updateAllTimeHistory(anlagenId));
}
	...
	return adapter.apiQueue.add(async () => {
		...
		for (let attempt = 0; attempt < maxAttempts; attempt++) {
			try {
				...
				if (status === 429) {
					adapter.log.warn("🚦 API returned 429 (rate limited). Backoff handled by AdaptiveRequestQueue.");
				}
