...
async function authStep(step, run) {
	try {
		...
		const err = new Error(`${step} → ${describeAuthFailure(e)}`, { cause: e });
...
async function apiLogin(adapter) {
	...
	try {
		...
		const pageRes = await authStep(`GET ${CONFIG.authUrl} (authorization request)`, () =>
		...
		const postForm = (step, url, data) =>
			authStep(`POST ${redactAuthUrl(url)} (${step})`, () =>
		...
		loginRes = await postForm(
			hasUsernameAndPassword(pageRes.data) ? "credentials" : "username",
			actionUrl,
			formData,
		);
		...
		if (!hasUsernameAndPassword(pageRes.data)) {
			...
			loginRes = await postForm("password", actionUrl, formData);
		}
		...
		if (!loginRes.headers.location && loginRes.status === 200 && loginRes.data && hasOtp(loginRes.data)) {
			...
			loginRes = await postForm("TOTP", otpAction, new URLSearchParams({ otp: otpCode }));
		}
		...
		const tokenRes = await authStep(`POST ${CONFIG.tokenUrl} (authorization_code exchange)`, () =>
			authClient.post(
				CONFIG.tokenUrl,
				new URLSearchParams({
	...
	adapter.refreshPromise = (async () => {
		...
		try {
			...
			if (errorCode === "invalid_grant") {
				adapter.log.debug(`[API] 🔐 Stored refresh token no longer accepted: ${describeAuthFailure(err)}`);
			} else {
				adapter.log.warn(`[API] ⚠️ Token refresh failed: ${describeAuthFailure(err)}`);
			}
