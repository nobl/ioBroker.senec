...
/**
 * Name the account the login is attempted with, without writing it out.
 *
 * "Which address does the adapter actually send?" is a question that a failing login cannot be
 * diagnosed without and that the log could not answer: the mail address is masked everywhere
 * else on purpose, so a report of a rejected login never showed whether the configured value is
 * the one the user believes it to be. The domain and the first character carry enough to compare
 * two addresses against each other while still not being the address.
 *
 * @param {unknown} mail - the configured account address
 * @returns {string} something like `u***@example.com`, or a note when there is nothing to name
 */
function maskMailAddress(mail) {
	if (typeof mail !== "string" || !mail.trim()) {
		return "(not configured)";
	}
	const value = mail.trim();
	const at = value.lastIndexOf("@");
	// No `@` is a misconfiguration worth seeing as such, and the value is then not an address
	// whose domain could be kept — so only its first character survives.
	if (at < 1) {
		return `${value[0]}***`;
	}
