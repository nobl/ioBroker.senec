...
async function doRebuild(adapter, anlagenId) {
	if (adapter.rebuildRunning) {
		adapter.log.debug(`Rebuild already running — skipping overlapping execution.`);
		return;
	}

	await adapter.initializeForcedRebuildIfNeeded();
	...
	try {
		...
		if (pendingSteps.length === 0) {
			if (await isRebuildFinishedForSystem(adapter, anlagenId)) {
				adapter.log.debug(`✅ Rebuild bereits vollständig für Anlage ${anlagenId}.`);
			} else {
				adapter.log.debug(`✅ Aktuell keine Rebuild-Schritte fällig für Anlage ${anlagenId}.`);
			}
			return;
		}
		...
		adapter.log.info(`Rebuild progress für Anlage ${anlagenId}: ${doneSteps}/${totalSteps} (${percent}%)`);

		if (await isRebuildFinishedForSystem(adapter, anlagenId)) {
			adapter.log.info(`✅ Rebuild completed for system: ${anlagenId}.`);
...
async function updateAllTimeHistory(adapter, anlagenId) {
	...
	adapter.log.debug(`Calculated AllTimeHistory: ${JSON.stringify(result)}`);
...
function logRebuildPendingFailuresIfChanged(adapter) {
	...
	if (summary && summary !== adapter.lastLoggedRebuildPendingSummary) {
		...
		adapter.log.info(`ℹ️ Noch offene Rebuild-Schritte: ${summary}`);
	}
...
async function runSingleRebuildStep(adapter, anlagenId, step) {
	...
	for (let attempt = 1; attempt <= adapter.rebuildStepMaxRetries; attempt++) {
		try {
			...
			const result = await adapter.doMeasurementsYear(anlagenId, step.year, step.monthly, step.wallbox);
			...
			if (result?.status === "success" || result?.status === "skipped_existing") {
				...
				adapter.log.info(`✅ Rebuild step successful: System ${anlagenId} / ${stepLabel}`);
				return true;
			}
			...
			if (result?.status === "no_data") {
				...
				adapter.log.info(`✅ Rebuild step completed with no data: System ${anlagenId} / ${stepLabel}`);
				return true;
			}
